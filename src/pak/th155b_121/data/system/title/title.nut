...
function UpdateMain()
{
	...
	if (this.cursor.ok)
	{
		...
		if (this.item[this.cursor.val] in this.proc)
		{
			this.proc[this.item[this.cursor.val]].call(this);
		}
	}
	else if (this.cursor.cancel)
	{
		this.cursor.val = this.item.len() - 1;
	}
...
{
	this.cursor_difficulty.Update();

	if (this.cursor_difficulty.ok)
	{
		if (this.cursor.val == 0)
		{
			if (this.cursor_difficulty.val != ::config.difficulty.story)
			{
				::config.difficulty.story = this.cursor_difficulty.val;
				::config.Save();
			}
		...
		{
			if (this.cursor_difficulty.val != ::config.difficulty.vs)
			{
				::config.difficulty.vs = this.cursor_difficulty.val;
				::config.Save();
			}
	...
	else if (this.cursor_difficulty.cancel)
	{
		this.Update = this.UpdateMain;
	}
