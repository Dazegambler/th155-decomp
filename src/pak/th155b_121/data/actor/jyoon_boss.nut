		...
		if (this.flag5.shotCount > this.count)
		{
			if (this.count >= 0 && this.count % this.flag5.shotCycle == 1)
			{
				...
				for( local i = 0; i < this.flag5.shotWay; i++ )
				{
					...
					this.SetLightShot(this.x + pos_.x * this.direction, this.y + pos_.y, 1.00000000, this.Boss_Shot_MS1, t_);
...
function Master_Spell_2_Attack()
{
	...
	this.func = [
		...
		function ()
		{
			this.SetLightShot(this.x - 32 + this.rand() % 64, ::battle.scroll_bottom, 1.00000000, this.Boss_Shot_MS2_High, {});
			this.SetLightShot(this.x - 32 + this.rand() % 64, ::battle.scroll_bottom, 1.00000000, this.Boss_Shot_MS2, {});
		}
	];
