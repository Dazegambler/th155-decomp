...
function CPU_CenterWaitFar( wait_ )
{
	...
	this.com_subState[2] = function ()
	{
		if (this.com_count[2] > 0 || this.centerStop * this.centerStop >= 4 || this.direction == 1.00000000 && this.com_keyFlag.x < 0 && this.x < ::battle.corner_left + 50 || this.direction == -1.00000000 && this.com_keyFlag.x > 0 && this.x > ::battle.corner_right - 50)
		{
			return true;
		}
		...
		if (this.com_rand[2] <= 3)
		{
			if (this.com_keyFlag.x >= 0 && this.targetDist <= (this.direction == 1.00000000 ? 400 : -800))
			{
				this.com_keyFlag.x = -1;
			}

			if (this.com_keyFlag.x <= 0 && this.targetDist >= (this.direction == 1.00000000 ? 800 : -400))
			{
				this.com_keyFlag.x = 1;
			}
		}
