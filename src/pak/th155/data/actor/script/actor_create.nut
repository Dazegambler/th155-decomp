...
function SetFreeObject( x_, y_, dir_, init_, t_, pare_ = null )
{
	t_.init <- init_;
	t_.pare <- pare_;
	return this.CreateActor2D(this.player_effect_class, x_, y_, dir_, this.player_effect_class.FreeObjectCommonInit, t_);
}
...
function SetShot( x_, y_, dir_, init_, t_, pare_ = null )
{
	t_.init <- init_;
	t_.pare <- pare_;
	return this.CreateActor2D(this.shot_class, x_, y_, dir_, this.shot_class.ShotCommonInit, t_);
}
