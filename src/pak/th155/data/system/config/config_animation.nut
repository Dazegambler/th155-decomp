...
function Initialize()
{
	...
	local item_left = 200;
	this.select_obj <- [];
	local select_x = left + item_left + 160;
	...
	foreach( v in t )
	{
		...
		this.select_obj.append(this.UIItemSelectorSingle(item_table[v].slice(1), select_x, text_table[v].y, this.mat_world, this.action["cursor_" + v]));
	...
	this.bar_src <- res;
	this.se_bar.x = se_back.x = text_table.se.x + item_left + 40;
