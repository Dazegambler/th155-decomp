...
{
	this.battleUpdate = this.Game_BeginUpdate;
	this.state = 2;
	this.demoCount = 0;
	this.endWinDemo = [
		false,
	...
	{
		this.team[0].current.func_beginDemo();
	}

	if (this.team[1].current.func_beginDemo)
	{
		this.team[1].current.func_beginDemo();
	}
...
function Game_BeginUpdate()
{
	...
	if (this.demoCount >= 60)
	{
		...
		if (c0.rsv_k0 + c0.rsv_k1 + c0.rsv_k2 + c0.rsv_k3 != 0 || c1.rsv_k0 + c1.rsv_k1 + c1.rsv_k2 + c1.rsv_k3 != 0)
		{
			this.RoundBeginDemo_Skip();
			return;
		}
...
function RoundBeginDemo_Skip()
{
	this.battleUpdate = this.Game_BeginSkipUpdate;
	this.state = 2;
	this.demoCount = 0;
	::graphics.FadeOut(10);
}

function Game_BeginSkipUpdate()
{
	this.demoCount++;

	if (this.demoCount == 10)
	{
		this.team[0].current.func_beginDemoSkip();
...
{
	this.battleUpdate = this.Game_BattleReady;
	this.state = 4;
	this.demoCount = 0;
	this.round++;
...
function Game_BattleReady()
{
	...
	if (this.demoCount == 20 + 100)
	{
		this.PlaySE(851);
		::actor.SetEffectLight(640, 360, 1001);
	}

	if (this.demoCount == 20 + 148)
	{
		this.PlaySE(853);
	}

	if (this.demoCount == 20 + 100 + 70)
	{
		this.Round_Fight();
	}
}
...
function Game_BattleUpdate()
{
	...
	if (!this.is_time_stop && this.enableTimeCount && this.time >= 0)
	{
		...
		if (this.time > 0 && this.time < this.time_unit * 10 && this.time % this.time_unit == 0)
		{
			::sound.PlaySE(867);
		...
		if (this.time == 0 && this.enableTimeUp)
		{
			this.Round_TimeUP();
		}
	}
...
function Round_KO()
{
	...
	if (this.winner == 0)
	{
		this.battleUpdate = this.Game_DowbleKoUpdate;
	}
	else if (this.win[0] >= this.match_num || this.win[1] >= this.match_num)
	{
		this.battleUpdate = this.Game_FinishUpdate;
	}
	else
	{
		this.battleUpdate = this.Game_KoUpdate;
	}
}

function Game_KoUpdate()
{
	this.demoCount++;
...
function Game_DowbleKoUpdate()
{
	this.demoCount++;

	if (this.demoCount == 210)
	{
		this.team[0].ResetCombo();
		this.team[1].ResetCombo();
		local t_ = {};
	...
	if (this.demoCount == 330)
	{
		this.BeginNextRound();
	}
}

function Round_TimeUP()
{
	this.FightEndFunction();
	...
	if (this.team[0].current.resetTimeUpFunc)
	{
		this.team[0].current.resetTimeUpFunc();
	}

	if (this.team[1].current.resetTimeUpFunc)
	{
		this.team[1].current.resetTimeUpFunc();
	}
}
...
function Game_TimeUpUpdate()
{
	if ((this.team[0].current.IsFree() && this.team[0].current.centerStop == 0) && this.team[1].current.IsFree() && this.team[1].current.centerStop == 0)
	{
		this.battleUpdate = function ()
		{
			...
			if (this.demoCount >= 150)
			{
				this.Round_TimeUpCall();
				return;
			}
		};
	}
...
function Round_TimeUpCall()
{
	...
	case 0:
		...
		if (w_.team.current == w_.team.slave)
		{
			w_.team.current.Team_Lose();
		}
		else if (w_.func_timeDemo)
		{
			w_.func_timeDemo();
		}

		if (l_.team.current == l_.team.slave)
		{
			l_.team.current.Team_Lose();
		}
		else if (l_.func_timeDemo)
		{
			l_.func_timeDemo();
		}
...
function Game_TimeUpCallUpdate()
{
	...
	{
		local c = this.team[0].current.command;
...
function Game_DrawUpdate()
{
	if (this.endLoseDemo[0] && this.endLoseDemo[1])
	{
		this.Round_DrawCall();
		return;
	}
...
function Round_DrawCall()
{
	this.battleUpdate = this.Game_DrawCallUpdate;
	this.state = 32;
	this.demoCount = 0;
	this.skipDemo = [
		false,
...
function Round_Win()
{
	this.battleUpdate = this.Game_WinUpdate;
	this.state = 32;
	this.demoCount = 0;
	this.endWinDemo = [
		false,
...
function Game_WinCallUpdate()
{
	...
	if (this.demoCount == 60)
	{
		if (this.win[0] >= this.match_num || this.win[1] >= this.match_num)
		{
			if (this.win[0] >= this.match_num && this.team[0].change_count == 0 && this.team[0].current.cpuState == null)
			{
				::trophy.SoloWin(0);
			}

			if (this.win[1] >= this.match_num && this.team[1].change_count == 0 && this.team[1].current.cpuState == null)
			{
				::trophy.SoloWin(1);
			}
...
{
	this.battleUpdate = this.Game_BeginNextRoundUpdate;
	this.state = 128;
	this.demoCount = 0;
...
function Game_BeginNextRoundUpdate()
{
	this.demoCount++;
...
{
	this.battleUpdate = this.Game_ResultUpdate;
	this.demoCount = 0;
...
function BeginResultEnd()
{
	this.battleUpdate = this.Game_ResultEndUpdate;
}

function Game_ResultEndUpdate()
{
	local b = false;
...
function SetWinner()
{
	if (this.team[0].life <= 0 && this.team[1].life <= 0)
	{
		this.winner = 0;
	}
	else if (this.team[0].life <= 0)
	{
		this.winner = 2;
	}
	else if (this.team[1].life <= 0)
	{
		this.winner = 1;
	}
	else if (this.team[0].life > this.team[1].life)
	{
		this.winner = 1;
	}
	else if (this.team[0].life < this.team[1].life)
	...
	{
	case 1:
		this.gauge.SetWin(0, this.win[0]);
		this.win[0]++;
		break;

	case 2:
		this.gauge.SetWin(1, this.win[1]);
		this.win[1]++;
		break;

	default:
		if (this.win[0] >= this.match_num - 1 && this.win[1] >= this.match_num - 1)
		{
		}
		else
		{
