function Initialize()
{
	...
	{
		if (!::menu.character_select.anime)
		{
			return;
		}
	...
	{
		for( local i = 0; i < this.character_id.len(); i = ++i )
		{
			local v = this.take[1000 * j + 1000 + i];
			local t = ::manbow.Texture();
			v.id <- 1000 * j + 1000 + i;
			v.texture <- t;

			if (j == 0 && (::menu.character_select.t[0].cursor_master.val == i || ::menu.character_select.t[1].cursor_master.val == i))
			{
	...
	v.Update <- function ()
	{
		if (this.action.Update == this.action.UpdateCharacterSelect)
		{
	...
	v.Update <- function ()
	{
		foreach( i, v in this.action.device_id )
		{
			if (this.device[i])
			{
				continue;
	...
	if (::network.IsPlaying())
	{
		...
		for( local i = 0; i < 2; i = ++i )
		{
			...
			v.text.sx = v.text.sy = 0,66000003.0;
