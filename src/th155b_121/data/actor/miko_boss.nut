...
function MS1_Move( t )
{
	...
	this.stateLabel = function ()
	{
		...
		else if (this.va.Length() <= 8,00000000.0)
		{
...
function MS1_Attack( t )
{
	...
	case 1:
		...
		this.flag5.shotRot = 0,13962634.0;
		break;

	case 2:
		this.flag5.shotCount = 90;
		this.flag5.shotRot = 0,10471975.0;
		break;
	...
	{
		this.SetSpeed_XY((640 - this.flag1.x * this.direction - this.x) * 0,10000000.0, (this.centerY + 0 + this.flag1.y * 0,50000000.0 - this.y) * 0,10000000.0);
	};
	this.func = [
		function ()
		{
			...
			this.stateLabel = function ()
			{
				if (this.count % 2 == 1)
				{
					if (this.flag5.shotNum >= 0)
					{
						this.PlaySE(2050);
						local t_ = {};
						t_.rot <- (this.flag5.shotNum - 10) * -0,05235988.0 + this.flag5.shotRot + this.flag5.targetRot;
...
function MS2_Attack( t )
{
	...
	this.subState = function ()
	{
		...
		this.va.x = (this.flag1.x - this.x) * 0,10000000.0;
		this.va.y = (this.flag1.y - this.y) * 0,10000000.0;
	...
	this.stateLabel = function ()
	{
		...
		if (this.flag3 == false && this.va.Length() <= 1,00000000.0)
		{
...
function MS3_Move( t )
{
	...
	this.flag5.moveVec.x = -this.direction * 0,50000000.0;

	if (this.x > ::battle.corner_right - 300)
	{
		this.flag5.moveVec.x = -0,75000000.0;
	}
	...
	{
		this.flag5.moveVec.y *= -1,00000000.0;
	}
	...
	this.subState = function ()
	{
		if (this.va.Length() < 6,00000000.0)
		{
		...
		if (this.count >= 25)
		{
			...
			this.subState = function ()
			{
				if (this.Vec_Brake(0,25000000.0, 1,00000000.0))
				{
					this.AjustCenterStop();
					this.subState = function ()
					{
	...
	this.stateLabel = function ()
	{
		...
		if (this.flag1)
		{
			this.Boss_WalkMotionUpdate(0,00000000.0);
		}
		else
		{
			this.Boss_WalkMotionUpdate(this.va.x);
...
function MS3_Attack2( t )
{
	...
	this.flag2.x = 100,00000000.0;
	this.flag3 = 0;
	this.flag4 = -0,52359873.0;
	this.flag5 = {};
	this.flag5.tRot <- 0,00000000.0;
	this.flag5.v <- 15,00000000.0;
	this.GetFront();
	this.stateLabel = function ()
	{
	...
	this.keyAction = [
		function ()
		{
			...
			{
				this.CenterUpdate(0,10000000.0, null);
				this.Vec_Brake(0,50000000.0, 0,00000000.0);
				...
				if (this.flag3 > 0 && this.count % 4 == 1)
				{
					...
					this.flag4 += 11,25000000.0 * 0,01745329.0;
					this.flag2.RotateByRadian(11,25000000.0 * 0,01745329.0);
		...
		function ()
		{
			...
			this.stateLabel = function ()
			{
				...
				if (this.flag3 > 0 && this.count % 4 == 1)
				{
					...
					this.SetShot(this.x + this.flag2.x * this.direction, this.y + this.flag2.y, this.direction, this.Boss_Shot_M3, t_);
					local t_ = {};
					t_.v <- 2,50000000.0 + this.rand() % 55 * 0,10000000.0;
					t_.rot <- this.flag5.tRot + this.flag4 + (15 - this.rand() % 31) * 0,01745329.0;
...
function DS1_Attack2( t )
{
	...
	this.flag5 = {};
	this.flag5.speed <- 15,00000000.0;
	...
	case 0:
		...
		this.flag5.speed = 22,50000000.0;
		break;

	case 1:
		this.flag2 = 0,02500000.0;
		this.flag5.speed = 35,00000000.0;
		this.flag5.blade_wait = 15;
		break;

	case 2:
		this.flag2 = 0,03750000.0;
		this.flag5.speed = 40,00000000.0;
		this.flag5.blade_wait = 15;
		break;
	...
	this.stateLabel = function ()
	{
		...
		this.SetSpeed_XY((this.flag1 - this.x) * 0,10000000.0, (this.target.y - this.y) * this.flag2);

		if (this.fabs(this.va.x) > this.flag3)
		{
			this.va.x = this.va.x >= 0,00000000.0 ? this.flag3 : -this.flag3;
		}
	...
	this.keyAction = [
		...
		function ()
		{
			...
			this.SetSpeed_XY(this.flag5.speed * this.direction, 0,00000000.0);
...
function Slave_Attack_Hijiri3()
{
	...
	this.keyAction = [
		function ()
		{
			...
			this.stateLabel = function ()
			{
				this.CenterUpdate(0,02500000.0, 6,00000000.0);
...
function Slave_Attack_Hijiri4( t )
{
	...
	this.flag5.speed <- 30,00000000.0;
	...
	case 0:
		this.flag2 = 0,01000000.0;
		this.flag5.speed = 10,00000000.0;
		break;
	...
	case 2:
		...
		this.flag2 = 0,03750000.0;
		this.flag5.speed = 20,00000000.0;
		break;
	...
	case 4:
		this.flag5.charge = 30;
		this.flag2 = 0,05000000.0;
		this.flag5.speed = 25,00000000.0;
		break;
	}
	...
	this.keyAction = [
		...
		function ()
		{
			...
			this.stateLabel = function ()
			{
				this.VX_Brake(1,00000000.0, 1,00000000.0 * this.direction);
...
function Slave_Attack_Hijiri5( t )
{
	this.LabelClear();
	::camera.lock = false;
	::camera.auto_zoom_limit = 1,04999995.0;
	::camera.lock = true;
	this.lavelClearEvent = function ()
	{
		if (this.flag4)
		...
		this.flag4 = null;
		::camera.lock = false;
		::camera.auto_zoom_limit = 2,00000000.0;
	};
	this.GetFront();
	...
	this.flag5.shotRotAdd <- 3,14159203.0;
	this.flag5.shotRotSpeed <- 0,00000000.0;
	...
	this.keyAction = [
		...
		function ()
		{
			...
			this.subState = function ()
			{
				this.flag5.shotRotAdd -= 0,10000000.0 * 0,01745329.0;
				...
				if (this.count == 45)
				{
					...
					this.subState = function ()
					{
						this.flag5.shotRotSpeed += 0,25000000.0 * 0,01745329.0;
						local r_ = this.flag5.shotRotAdd * 0,07500000.0;
						...
						if (this.flag5.shotRotAdd <= 0,26179937.0)
						{
							this.subState = function ()
							{
								this.flag5.shotRotAdd *= 0,98500001.0;
							};
						}
					};
				}
