...
function BeginBattleA( t )
{
	...
	this.SetSpeed_XY(10,00000000.0 * this.direction, 18,00000000.0);
	this.LabelClear();
	this.count = 0;
	this.PlaySE(1290);
	...
	this.stateLabel = function ()
	{
		...
		if (this.va.y <= 0,00000000.0)
		{
			...
			{
				this.func();
				local v_ = (::battle.start_x[this.flag1] - this.x) * this.direction * 0,10000000.0;
				...
				if (this.va.y < -7,00000000.0)
				{
					this.stateLabel = function ()
					{
						...
						if (this.abs(::battle.start_x[this.flag1] - this.x) <= 0,50000000.0)
						{
						...
						this.CenterUpdate(0,50000000.0, 5,00000000.0);
					};
				}
			};
		}
...
function MoveBack_Init( t )
{
	...
	this.SetMotion(2, 0);
	this.SetSpeed_XY(-5,75000000.0 * this.direction, this.va.y);
}
...
function Team_Change_AirMoveB( t_ )
{
	...
	this.flag5.vx = 8,50000000.0;
	this.flag5.vy = 5,00000000.0;
	this.flag5.g = 0,50000000.0;
}

function Team_Change_AirBackB( t_ )
{
	this.Team_Change_AirBackCommon(null);
	this.flag5.vx = -7,50000000.0;
		...
		this.keyAction = [
			function ()
			{
				...
				this.stateLabel = function ()
				{
					...
					this.AddSpeed_XY(0,34999999.0 * this.direction, 0,00000000.0);

					if (this.va.x * this.direction > 16,00000000.0)
					{
						this.SetSpeed_XY(16,00000000.0 * this.direction, null);
					}
				};
			}
		];
...
{
	local t_ = {};
	t_.speed <- 10,00000000.0;
	t_.g <- 0,10000000.0;
	t_.minWait <- 20;
	t_.wait <- 60;
	t_.addSpeed <- 0,15000001.0;
...
function DashBack_Init( t )
{
	...
	this.keyAction = [
		...
		{
			this.PlaySE(801);
			this.SetSpeed_XY(-13,50000000.0 * this.direction, -4,00000000.0);
			this.centerStop = -2;
			this.stateLabel = function ()
			{
...
function Atk_Mid_Init( t )
{
	...
	this.keyAction = [
		function ()
		{
			...
			if (this.va.x * this.direction <= 5,00000000.0)
			{
	...
	else
	{
		...
		this.keyAction = [
			function ()
			{
				this.PlaySE(1208);
				this.centerStop = 2;
				this.SetSpeed_XY(14,00000000.0 * this.direction, 8,00000000.0);
				this.stateLabel = function ()
				{
...
function Atk_HighUpper_Init( t )
{
	...
	this.keyAction = [
		...
		function ()
		{
			this.SetSpeed_XY(6,00000000.0 * this.direction, -7,50000000.0);
			this.centerStop = -2;
			this.stateLabel = function ()
			{
				this.VX_Brake(0,40000001.0);
...
function Atk_HighUpper_Charge()
{
	...
	this.keyAction = [
		function ()
		{
			...
			this.PlaySE(1210);
			this.SetSpeed_XY(8,00000000.0 * this.direction, -14,00000000.0);
			this.centerStop = -2;
			this.stateLabel = function ()
			{
				this.VX_Brake(0,40000001.0);
...
function Atk_RushC_Front_Init( t )
{
	...
	this.keyAction = [
		function ()
		{
			...
			this.stateLabel = function ()
			{
				this.VX_Brake(this.va.x * this.direction >= 1,00000000.0 ? 0,50000000.0 : 0,05000000.0);
			};
		},
		function ()
		{
	...
	this.keyAction = [
		function ()
		{
			...
			if (this.target.centerStop * this.target.centerStop >= 2)
			{
				...
				t_.rot = this.Math_MinMax(t_.rot, -0,26179937.0, 0,26179937.0);
			}
...
function Shot_Front_Init( t )
{
	...
	this.keyAction = [
		function ()
		{
			...
			this.stateLabel = function ()
			{
				this.VX_Brake(this.va.x * this.direction < -1,00000000.0 ? 0,25000000.0 : 0,00000000.0);
			};
		},
		function ()
		{
...
function Shot_Front_Air_Init( t )
{
	...
	this.keyAction = [
		function ()
		{
			...
			{
				this.VX_Brake(this.va.x * this.direction < -1,00000000.0 ? 0,25000000.0 : 0,00000000.0);
				this.CenterUpdate(0,10000000.0, 3,00000000.0);
...
function Shot_Charge_Fire( t )
{
	...
	this.keyAction = [
		function ()
		{
			...
			this.SetSpeed_XY(-7,50000000.0 * this.direction, -10,00000000.0);
			this.centerStop = -2;
			this.stateLabel = function ()
			{
...
function Shot_Burrage_Init( t )
{
	...
	this.flag2.rot_speed <- 13 * 0,01745329.0;
	...
	this.subState = function ()
	{
		if (this.team.mp > 0)
		{
			if (this.count >= 10 && this.count % 2 == 0)
			{
				...
				local t_ = {};
				t_.v <- this.Vector3();
				t_.v.x = this.flag2.pos.x * 0,20000000.0 * this.direction;
				t_.v.y = this.flag2.pos.y * 0,20000000.0;
...
function Okult_PreAura( t )
{
	...
	this.stateLabel = function ()
	{
		this.sx = this.sy += (0,60000002.0 - this.sx) * 0,20000000.0;

		if (this.sx >= 0,50000000.0)
		{
			this.stateLabel = function ()
			{
				this.sx = this.sy += (0,33000001.0 - this.sx) * 0,00500000.0;
				this.subState();
			};
		}
...
function SP_A_Init( t )
{
	...
	this.keyAction = [
		...
		{
			this.team.AddMP(-200, 120);
			this.SetSpeed_Vec(25,00000000.0, 0,00000000.0, this.direction);
			this.PlaySE(1300);
...
function SP_A_Air_Init( t )
{
	...
	this.keyAction = [
		function ()
		{
			this.AjustCenterStop();
			this.team.AddMP(-200, 120);
			this.SetSpeed_Vec(20,00000000.0, 0,00000000.0, this.direction);
			this.PlaySE(1300);
			this.flag4 = this.SetFreeObject(this.x, this.y, this.direction, this.SP_A_Bloom, {}).weakref();
			this.lavelClearEvent = function ()
			{
...
function SP_B_Init( t )
{
	...
	this.keyAction = [
		...
		{
			this.team.AddMP(-200, 120);
			this.flag1 = -3,14159203.0 / 30 * this.direction;
		...
		function ()
		{
			...
			this.stateLabel = function ()
			{
				...
				if (this.count >= 10)
				{
					this.SetSpeed_XY(0,00000000.0, this.va.y * 0,85000002.0);
				}
				...
				if (this.count >= 20)
				{
					...
					this.stateLabel = function ()
					{
						this.AddSpeed_XY(0,00000000.0, 0,75000000.0, null, 12,50000000.0);
					};
				}
			};
			local t_ = {};
		...
		function ()
		{
			...
			this.stateLabel = function ()
			{
				...
				if (this.count >= 6 && this.y >= this.centerY)
				{
					this.centerStop = 1;
					this.stateLabel = null;
					this.SetSpeed_XY(null, 3,50000000.0);
...
function SP_C_Init( t )
{
	...
	else if (this.y < this.centerY)
	{
		...
		this.func = function ()
		{
			...
			this.PlaySE(1305);
			local t_ = {};
			t_.rot <- 20,00000000.0 * 0,01745329.0;
			this.SetShot(this.point0_x, this.point0_y, this.direction, this.SPShot_C, t_);
		};
	}
	else
			...
			this.stateLabel = function ()
			{
				...
				if (this.count % 5 == 0)
				{
					if (this.flag3 > 0)
					{
						...
						t_.count <- 15;
						t_.posY <- this.flag4;
						t_.rot <- (15,00000000.0 - this.flag3 * 4) * 0,01745329.0;
...
function SP_D_Air_Init( t )
{
	...
	{
		this.flag5 = this.va.x > 0,00000000.0 ? 2,00000000.0 : -2,00000000.0;
	}
	else
	{
	...
	this.keyAction = [
		...
		function ()
		{
			...
			this.stateLabel = function ()
			{
				...
				{
					this.VX_Brake(0,50000000.0, this.flag5);
				}
		...
		this.keyAction = [
			function ()
			{
				...
				this.stateLabel = function ()
				{
					...
					this.AddSpeed_Vec(0,75000000.0, 30 * 0,01745329.0, 17,50000000.0, this.direction);
			...
			function ()
			{
				this.SetSpeed_XY(this.va.x * 0,75000000.0, this.va.y * 0,50000000.0);
				this.stateLabel = function ()
				{
					this.CenterUpdate(0,25000000.0, null);
				};
...
function SP_F_Init( t )
{
	...
	if (this.centerStop * this.centerStop <= 1)
	{
		...
		this.flag1.x = 15,00000000.0;
		this.flag1.y = -10,00000000.0;
	...
	else
	{
		...
		if (this.y > this.centerY)
		{
			this.flag1.x = 6,00000000.0;
			this.flag1.y = -14,50000000.0;
		}
		else
		{
			this.flag1.x = 6,00000000.0;
			this.flag1.y = -3,50000000.0;
		}
	}
	...
	this.keyAction = [
		...
		function ()
		{
			...
			this.stateLabel = function ()
			{
				...
				this.CenterUpdate(0,50000000.0, 1,00000000.0);
				this.VX_Brake(0,50000000.0);
...
function Spell_B_Init( t )
{
	...
	this.keyAction = [
		function ()
		{
			this.freeMap = true;
			this.UseSpellCard(60, -this.team.sp_max);
			this.SetSpeed_XY(-60,00000000.0 * this.direction, 0,00000000.0);
			this.stateLabel = function ()
			{
				if (this.direction == 1,00000000.0 && this.x < ::battle.scroll_left - 240,00000000.0 || this.direction == -1,00000000.0 && this.x > ::battle.scroll_right + 240,00000000.0)
				{
					if (this.direction == 1,00000000.0)
					{
						this.Warp(::battle.scroll_left - 240,00000000.0, this.y);
					}
					else
					{
						this.Warp(::battle.scroll_right + 240,00000000.0, this.y);
					}
		...
		function ()
		{
			...
			this.PlaySE(1359);
			this.SetSpeed_XY(80,00000000.0 * this.direction, 0,00000000.0);
			this.stateLabel = function ()
			{
				...
				if (this.direction == 1,00000000.0 && this.x >= ::battle.scroll_right + 1280 || this.direction == -1,00000000.0 && this.x <= ::battle.scroll_left - 1280)
				{
		...
		function ()
		{
			...
			if (this.direction == 1,00000000.0)
			{
				this.Warp(::battle.scroll_left - 120,00000000.0, ::battle.scroll_top - 120);
			}
			else
			{
				this.Warp(::battle.scroll_right + 120,00000000.0, ::battle.scroll_top - 120);
			}
			...
			this.stateLabel = function ()
			{
				...
				if (this.IsCenter(20,00000000.0) == 0)
				{
...
function Spell_Climax_Init( t )
{
	...
	this.keyAction = [
		function ()
		{
			this.UseClimaxSpell(60, "–ƒXƒeƒLI™\x2560‚\x2560‰ÔŽq‚\x2502‚ñI–");
			::battle.enableTimeUp = false;
			this.lavelClearEvent = function ()
			{
		...
		function ()
		{
			this.SetSpeed_XY(8,00000000.0 * this.direction, -12,50000000.0);
			this.centerStop = -2;
			this.HitTargetReset();
			this.stateLabel = function ()
			{
...
function Spell_Climax_Hit( t )
{
	...
	this.stateLabel = function ()
	{
		...
		this.GetPoint(0, this.flag1);
		this.VX_Brake(this.va.x * this.direction <= -1,00000000.0 ? 0,50000000.0 : 0,00500000.0);
	...
	this.keyAction = [
		...
		function ()
		{
			...
			this.flag2.y = 10,00000000.0;
			this.stateLabel = function ()
			{
				this.VX_Brake(0,50000000.0);
				this.flag2.y += 0,25000000.0;
				this.target.y += this.flag2.y;

				if (this.target.y > this.y - 50)
				{
					this.PlaySE(1273);
					::camera.shake_radius = 25,00000000.0;
					...
					{
						this.VX_Brake(0,50000000.0);
						this.target.sx = this.target.sy += 0,30000001.0;
						this.target.masterRed = this.target.masterGreen = this.target.masterBlue -= 0,02500000.0;
						this.target.x = this.flag3.x + this.flag4.x * ((this.target.sx - 1,00000000.0) * 0,25000000.0);
						this.target.y = this.flag3.y + this.flag4.y * ((this.target.sy - 1,00000000.0) * 0,25000000.0);
						this.target.rz -= 24,00000000.0 * 0,01745329.0;
...
function Spell_Climax_Finish( t )
{
	...
	this.FadeOut(0,00000000.0, 0,00000000.0, 0,00000000.0, 20);
	this.stateLabel = function ()
	{
		if (this.count == 40)
		{
			...
			this.flag1 = this.SetFreeObject(0, 0, 1,00000000.0, this.Climax_Back, {}).weakref();
			this.FadeIn(0,00000000.0, 0,00000000.0, 0,00000000.0, 10);
			this.count = 0;
			this.stateLabel = function ()
			{
				if (this.count == 60)
				...
				if (this.count == 160)
				{
					this.PlaySE(1276);
					this.KnockBackTarget(-this.direction);
					this.flag2 = this.SetFreeObject(640 + 190, 360 - 25, 1,00000000.0, this.Climax_CutInA, {}).weakref();
				}
...
function Spell_Climax_FinishB( t )
{
	...
	this.target.masterRed = this.target.masterGreen = this.target.masterBlue = 1,00000000.0;
	this.target.sx = this.target.sy = 1,00000000.0;
	::camera.ResetTarget();
