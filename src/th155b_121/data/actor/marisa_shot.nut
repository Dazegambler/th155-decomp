...
function BattleWinObject_A( t )
{
	...
	this.owner.demoObject.append(this.weakref());
	this.sx = this.sy = 1,25000000.0 + this.rand() % 15 * 0,10000000.0;
	this.rz = (360 - this.rand() % 720) * 0,01745329.0;
	this.red = this.rand() % 10 * 0,10000000.0;
	this.green = this.rand() % 10 * 0,10000000.0;
	this.blue = this.rand() % 10 * 0,10000000.0;
	this.SetSpeed_XY(1,00000000.0 - this.rand() % 21 * 0,10000000.0, -4,00000000.0 - this.rand() % 15 * 0,10000000.0);
	this.stateLabel = function ()
	{
		this.rz *= 0,98000002.0;
		this.AddSpeed_XY(0,00000000.0, 0,10000000.0);
	...
	this.SetFreeObject(this.x - 25 + this.rand() % 50, this.y - 25 + this.rand() % 50, this.direction, function ( t_ )
	{
		...
		this.green = this.rand() % 10 * 0,10000000.0;
		this.blue = this.rand() % 10 * 0,10000000.0;
		this.SetSpeed_XY(1,00000000.0 - this.rand() % 21 * 0,10000000.0, 1,00000000.0 - this.rand() % 21 * 0,10000000.0);
		this.stateLabel = function ()
		{
			this.rz *= 0,98000002.0;
			this.AddSpeed_XY(0,00000000.0, 0,01000000.0);
			this.count++;
			this.sx = this.sy *= 0,95999998.0;
...
function NormalShot( t )
{
	...
	this.SetTaskAddRotation(0,00000000.0, 0,00000000.0, 0,06981317.0);
	this.flag5 = this.SetTrail(2009, 6, 12, 75).weakref();

	for( local i = 2; i < 5; i++ )
	{
		local t_ = {};
		t_.rot <- i * 0,26179937.0;
	...
	this.func = [
		function ()
		{
			...
			this.stateLabel = function ()
			{
				if (this.flag5)
				{
					this.flag5.anime.radius0 *= 0,60000002.0;
				}
	...
	this.stateLabel = function ()
	{
		...
		this.sx = this.sy += (1,75000000.0 - this.sx) * 0,10000000.0;
...
function NormalShotMini( t )
{
	...
	this.rz = t.rot;
	this.flag2 = this.Vector3();
	this.flag2.x = 5,00000000.0 * this.direction;
	this.flag2.RotateByRadian(t.rot);
	this.SetSpeed_XY(this.flag2.x, this.flag2.y);
	this.SetTaskAddRotation(0,00000000.0, 0,00000000.0, 0,08726646.0);
	this.stateLabel = function ()
	{
		this.flag2.x += 0,20000000.0 * this.direction;

		if (this.fabs(this.flag2.y) <= 0,10000000.0)
		{
			this.flag2.y = 0,00000000.0;
		}
		else
		{
			this.flag2.y -= this.flag2.y > 0 ? 0,10000000.0 : -0,10000000.0;
		}
...
function NormalShot_Trail( t )
{
	...
	this.sx = this.sy = 0,50000000.0 + this.rand() % 20 * 0,10000000.0;
	this.SetSpeed_Vec(this.rand() % 30 * 0,10000000.0 + t.vec, t.rot + (this.rand() % 30 - 15) * 0,01745329.0, this.direction);
	this.stateLabel = function ()
	{
		this.rz += 0,10000000.0;
		this.Vec_Brake(0,30000001.0);

		if (this.alpha > 0,02500000.0)
		{
			this.alpha -= 0,02500000.0;
			this.green -= 0,02500000.0;
			this.red -= 0,02500000.0;
		}
		else
		{
			this.ReleaseActor();
...
function NormalShot_Exp( t )
{
	...
	this.stateLabel = function ()
	{
		...
		if (this.count >= 2)
		{
			...
			this.red = this.green -= 0,10000000.0;
...
function Shot_BottlePart( t )
{
	...
	this.sx = this.sy = 1,00000000.0 + this.rand() % 11 * 0,10000000.0;
	this.rz = this.rand() % 360 * 0,01745329.0;
	this.SetSpeed_XY(-1,00000000.0 * this.direction, 0,00000000.0);
	this.stateLabel = function ()
	{
...
function Shot_Bottle( t )
{
	...
	this.func = [
		function ()
		{
			...
			this.HitReset();
			this.PlaySE(1315);
			this.rz = 1,00000000.0;

			if (this.initTable.h > 0,00000000.0)
			{
			...
			this.stateLabel = function ()
			{
				...
				if (this.count == 3)
				{
					this.flag1 = -27,00000000.0 * 0,01745329.0;
					this.stateLabel = function ()
					{
						this.subState();
	...
	this.stateLabel = function ()
	{
		...
		this.AddSpeed_XY(null, 0,44999999.0);
	};
}
...
function Shot_Bottle_Star( t )
{
	...
	this.rz = this.rand() % 360 * 0,01745329.0;
	this.va.x = 13,50000000.0;
	this.va.RotateByRadian(t.rot);
	this.va.y -= 3,00000000.0;
	this.SetSpeed_XY(null, null);
	this.sx = this.sy += this.rand() % 25 * 0,01000000.0;
	this.flag1 = (5 - this.rand() % 11) * 0,01745329.0;
	this.cancelCount = 1;
	this.atk_id = 32768;
	this.func = [
		function ()
	...
	this.subState = function ()
	{
		if (this.Vec_Brake(0,75000000.0, 2,00000000.0))
		{
			this.subState = function ()
			{
				this.count++;
...
function Shot_Laser( t )
{
	...
	this.SetCollisionScaling(0,01000000.0, 1,00000000.0, 1,00000000.0);
...
function Shot_Barrage( t )
{
	...
	this.flag1.x = t.v.x * 0,02000000.0;
	this.flag1.y = t.v.y * 0,02000000.0;
	this.keyAction = this.ReleaseActor;
...
function Shot_ChargeFull_Star( t )
{
	...
	this.SetSpeed_XY(this.flag1.x * 2,50000000.0, this.flag1.y * 2,50000000.0);
	this.flag1.Mul(0,10000000.0);
	this.SetTaskAddRotation(0,00000000.0, 0,00000000.0, 5,00000000.0 * 0,01745329.0);
	this.stateLabel = function ()
	{
...
function Shot_ChargeFull( t )
{
	...
	this.flag3.x = 1,00000000.0;
	this.flag3.y = 0,00000000.0;
	...
	this.subState = function ()
	{
		this.AddSpeed_Vec(0,05000000.0, this.flag1, 45,00000000.0, this.direction);
		...
		if (this.count >= 60)
		{
			this.subState = function ()
			{
				...
				this.AddSpeed_Vec(0,50000000.0, this.flag1, 45,00000000.0, this.direction);
			};
		}
	};
	this.stateLabel = function ()
...
function Shot_Charge( t )
{
	...
	this.stateLabel = function ()
	{
		...
		this.count++;
		this.AddSpeed_Vec(1,00000000.0, this.flag1, 45,00000000.0, this.direction);
...
function Shot_ChargeTrail( t )
{
	...
	this.rz = 0,01745329.0 * this.rand() % 360;
	this.sx = this.sy = 0,20000000.0 + this.rand() % 20 * 0,10000000.0;
	this.SetSpeed_Vec(this.rand() % 30 * 0,10000000.0 + t.vec, t.rot + (this.rand() % 30 - 15) * 0,01745329.0, this.direction);
	this.stateLabel = function ()
	{
		this.rz += 0,10000000.0;
...
function Shot_Change( t )
{
	...
	this.SetSpeed_Vec(14,00000000.0, t.rot, this.direction);
	this.cancelCount = 1;
	...
	this.flag1 = this.va.y * 0,02500000.0;
	this.stateLabel = function ()
	{
...
function Shot_ChangeFin( t )
{
	...
	this.cancelCount = 3;
	this.atk_id = 536870912;
	this.SetSpeed_Vec(8,00000000.0, this.initTable.rot, this.direction);
	this.rz = t.rot;
...
function Shot_OkultA( t )
{
	...
	this.flag1 = 0;
	this.flag2 = null;
	this.SetSpeed_XY(15,00000000.0 * this.direction, -3,00000000.0);
	this.cancelCount = 3;
	this.atk_id = 524288;
	this.flag4 = 2;
	this.func = [
		...
		function ()
		{
			...
			this.stateLabel = function ()
			{
				this.AddSpeed_XY(this.va.x * this.direction <= 3,00000000.0 ? 1,50000000.0 * this.direction : 0,00000000.0, 0,00000000.0);
				this.alpha -= 0,10000000.0;
	...
	this.stateLabel = function ()
	{
		if (this.IsScreen(50,00000000.0))
		{
			this.ReleaseActor();
	...
	this.keyAction = [
		...
		function ()
		{
			...
			this.stateLabel = function ()
			{
				...
				if (this.IsScreen(100,00000000.0) || this.cancelCount <= 0 || this.owner.motion == 2500 && this.owner.keyTake == 0)
				{
					this.func[1].call(this);
					return;
				}
...
function Shot_OkultB( t )
{
	...
	this.stateLabel = function ()
	{
		...
		this.AddSpeed_XY(0,00000000.0, this.va.y < -0,75000000.0 ? 0,75000000.0 : 0,00000000.0);
...
function Shot_OkultD( t )
{
	...
	this.func = [
		function ()
		{
			...
			this.stateLabel = function ()
			{
				this.rz += (-160 * 0,01745329.0 - this.rz) * 0,05000000.0;
		...
		function ()
		{
			this.stateLabel = function ()
			{
				...
				if (this.Vec_Brake(1,00000000.0))
				{
	...
	this.stateLabel = function ()
	{
		...
		this.AddSpeed_XY(0,00000000.0, 0,80000001.0);
		this.count++;
...
function Shot_OkultD_FlashA( t )
{
	...
	this.sx = this.sy = 0,10000000.0;
	this.flag1 = -45,00000000.0 * 0,01745329.0;
	this.stateLabel = function ()
	{
		this.count++;
		this.rz += this.flag1;
		...
		if (this.count <= 15)
		{
			this.sx += (1,00000000.0 - this.sx) * 0,20000000.0;
			this.sy += (1,00000000.0 - this.sy) * 0,20000000.0;
		}
		else
		{
			this.sx += (2,00000000.0 - this.sx) * 0,10000000.0;
...
function Shot_OkultD_Beam( t )
{
	...
	this.stateLabel = function ()
	{
		...
		this.sx += (0,25000000.0 - this.sx) * 0,10000000.0;
...
function Shot_OkultE( t )
{
	...
	this.life = 1000;
	this.atk_id = 524288;
	this.SetSpeed_XY(5,00000000.0 * this.direction, -7,00000000.0);
	this.subState = function ()
	{
		if (this.life <= 0)
		{
...
function Shot_OkultE_Break( t )
{
	...
	this.flag1 = (-45 + this.rand() % 90) * 0,01745329.0;
	this.SetSpeed_XY(-2,00000000.0 + this.rand() % 5, -1,00000000.0 - this.rand() % 5);
	this.stateLabel = function ()
	{
		this.rz += (this.flag1 - this.rz) * 0,02500000.0;
		this.AddSpeed_XY(0,00000000.0, 0,50000000.0);
		this.alpha -= 0,02500000.0;
...
function Shot_OkultF( t )
{
	...
	this.func = [
		function ()
		{
			...
			this.stateLabel = function ()
			{
				...
				this.rz += (-3,14159203.0 - this.rz) * 0,07500000.0;
				this.alpha -= 0,05000000.0;
		...
		function ()
		{
			...
			this.stateLabel = function ()
			{
				...
				{
					this.VY_Brake(0,30000001.0);
				}

				this.AddSpeed_XY(this.va.x * this.direction <= 12,50000000.0 ? 0,50000000.0 * this.direction : 0,05000000.0, -0,25000000.0, null, -6,00000000.0);
	...
	this.stateLabel = function ()
	{
		...
		if (this.va.x > 0,00000000.0 && this.x > 1280 || this.va.x < 0,00000000.0 && this.x < 0)
		{
...
function SPShot_B_Blade( t )
{
	...
	this.stateLabel = function ()
	{
		...
		{
			this.sy = (this.flag1 - this.y) / 90,00000000.0;
...
function SPShot_C_Exp( t )
{
	...
	{
		local t_ = {};
		t_.rot <- (i * 18 + this.rand() % 45) * 0,01745329.0;
		t_.vec <- 6,00000000.0 + this.rand() % 12;
...
function SPShot_C2( t )
{
	...
	this.stateLabel = function ()
	{
		...
		if (this.hitResult)
		{
			this.SetSpeed_Vec(2,50000000.0, this.flag1, this.direction);
		}
		else
		{
			this.AddSpeed_Vec(1,00000000.0, this.flag1, 45,00000000.0, this.direction);
		...
		{
			local t_ = {};
			t_.rot <- this.flag1 + 3,14159203.0;
			t_.vec <- this.va.LengthXY() * 0,50000000.0;
			this.SetShot(this.x, this.y, this.direction, this.SPShot_C_Trail, t_);
		}
...
function SPShot_D( t )
{
	...
	this.SetSpeed_XY(0,00000000.0, -45,00000000.0);
	this.cancelCount = 1;
	this.atk_id = 8388608;
	this.stateLabel = function ()
	{
		...
		this.rz += 0,30000001.0;
		this.count++;

		if (this.count % 4 == 0)
		{
			local t_ = {};
			t_.rot <- 3,14159203.0 * 0,50000000.0;
			t_.vec <- this.va.LengthXY() * 0,50000000.0;
		...
		if (this.count >= this.initTable.count)
		{
			...
			{
				local t_ = {};
				t_.rot <- 90 * 0,01745329.0 + this.initTable.rot * (1 - i * 2);
...
function SPShot_D_Flash( t )
{
	...
	this.sx = this.sy = 1,00000000.0 + this.rand() % 20 * 0,10000000.0;
	this.stateLabel = function ()
	{
		this.sx = this.sy *= 0,92000002.0;
		...
		{
			this.alpha -= 0,10000000.0;
			this.green -= 0,10000000.0;
			this.red -= 0,10000000.0;
		}
		else
		{
			this.ReleaseActor();
...
function SPShot_D_Fall( t )
{
	...
	this.rz = t.rot;
	this.SetSpeed_Vec(37,50000000.0, this.rz, this.direction);
	this.cancelCount = 1;
	this.atk_id = 8388608;
	this.stateLabel = function ()
	{
...
function SPShot_D2_Fall( t )
{
	...
	local R_ = (100 - this.rand() % 20) * 0,01745329.0;
	this.SetSpeed_Vec(30,00000000.0 + this.rand() % 15, R_, this.direction);
	this.rz = R_;
	this.cancelCount = 1;
	this.atk_id = 8388608;
	this.stateLabel = function ()
...
function SPShot_E_Star( t )
{
	...
	this.rz = this.rand() % 360 * 0,01745329.0;
	this.sx = this.sy = 1,00000000.0 + this.rand() % 150 * 0,01000000.0;
	this.stateLabel = function ()
	{
...
function SPShot_F( t )
{
	...
	this.stateLabel = function ()
	{
		...
		if (this.va.y > 0,00000000.0 && this.y > ::battle.corner_bottom)
		{
			this.SetSpeed_XY(null, this.va.y * -0,75000000.0);
		}

		if (this.va.x > 0,00000000.0 && this.x > ::battle.corner_right || this.va.x < 0,00000000.0 && this.x < ::battle.corner_left)
		{
			this.SetSpeed_XY(this.va.x * -0,25000000.0, null);
		}
...
function SPShot_F_Star( t )
{
	...
	this.func = function ()
	{
		...
		this.stateLabel = function ()
		{
			...
			if (this.flag1 < 2,00000000.0)
			{
				this.flag1 = 2,00000000.0;
			}

			this.SetSpeed_Vec(this.flag1, this.flag2, this.direction);
			this.AddSpeed_XY(0,00000000.0, this.flag3);
			this.flag3 += 0,25000000.0;
	...
	this.func = [
		...
		function ()
		{
			...
			this.stateLabel = function ()
			{
				...
				this.Warp(this.x + (this.owner.point0_x - this.x) * 0,50000000.0, this.y + (this.owner.point0_y - this.y) * 0,50000000.0);
...
function SPShot_G_Shot( t )
{
	...
	this.rz = this.rand() % 360 * 0,01745329.0;
	this.SetSpeed_Vec(40,00000000.0, t.rot, this.direction);
	this.cancelCount = 1;
	this.SetTrail(6060, 2, 5, 10);
	this.PlaySE(1373);
	this.SetCollisionRotation(0,00000000.0, 0,00000000.0, t.rot);
	...
	this.stateLabel = function ()
	{
		...
		this.AddSpeed_Vec(3,50000000.0, null, 75, this.direction);
...
function SpellA_Shot( t )
{
	...
	{
		this.count++;
		this.sy += 0,00100000.0;
		this.SetCollisionScaling(this.sx, this.sy + this.flag1, 1,00000000.0);

		if (this.flag1 == 0,00000000.0 && this.hitResult & 1)
		{
...
function SpellA_Root( t )
{
	...
	this.stateLabel = function ()
	{
		if (this.flag1 in ::actor)
		{
			...
			this.sx = this.sy = ::actor[this.flag1].sy * 1,25000000.0;
...
function SpellB_Shot( t )
{
	...
	this.stateLabel = function ()
	{
		if (this.owner.motion == 4010)
		{
			...
			{
				if (this.owner.x < 1280 && this.direction == 1,00000000.0 || this.owner.x > 0 && this.direction == -1,00000000.0)
				{
					this.Warp(this.owner.x, this.owner.y + 50,00000000.0);
				}
				else
				{
					this.stateLabel = function ()
					{
						...
						this.sy += (2,00000000.0 - this.sy) * 0,15000001.0;
						this.alpha = this.red = this.green -= 0,01250000.0;
...
function SpellC_Shot( t )
{
	...
	this.stateLabel = function ()
	{
		...
		if (this.hitCount >= 6 || this.cancelCount <= 0 || this.count >= 60)
		{
			...
			this.stateLabel = function ()
			{
				this.SetSpeed_XY(this.va.x * 0,92000002.0, 0,00000000.0);
		...
		if (this.hitResult)
		{
			this.SetSpeed_XY(this.va.x * 0,69999999.0, 0,00000000.0);
			this.flag1++;
		...
		if (this.abs(this.va.x) <= 2,00000000.0)
		{
...
function SpellC_ShotCore( t )
{
	...
	{
		this.count++;
		this.x += (this.team.current.x - this.x) * 0,25000000.0;
		this.y += (this.team.current.y - this.y) * 0,25000000.0;
		...
		for( local i = 0; i < 6; i++ )
		{
			if (this.flag1[i])
			{
				...
				this.flag1[i].y = this.y + this.flag2.y * this.flag3 * 0,89999998.0;
				...
				else if (this.flag2.y > 0,00000000.0)
				{
					this.flag1[i].DrawActorPriority(200);
				}
			}

			this.flag2.RotateByDegree(60,00000000.0);
		}

		this.flag2.RotateByDegree(4,00000000.0);
		this.flag3 += (120 - this.flag3) * 0,10000000.0;
...
function Climax_CutB( t )
{
	...
	this.stateLabel = function ()
	{
		this.sx = this.sy += (1,25000000.0 - this.sx) * 0,00250000.0;
		this.SetSpeed_XY(0,03000000.0 * this.sx, -0,25000000.0 * this.sy);
...
function Climax_CutB_Exp( t )
{
	...
	this.stateLabel = function ()
	{
		this.sx = this.sy += (1,50000000.0 - this.sx) * 0,01000000.0;
...
function Climax_Back( t )
{
	...
	this.func = [
		...
		function ()
		{
			...
			t_.motion <- 303;
			t_.keyTake <- 1;
			this.flag3 = this.owner.target.SetFreeObject(this.x - 360, this.y + 180, -1,00000000.0, this.DummyPlayer, t_).weakref();
			this.stateLabel = function ()
			{
				this.flag3.x += 40;
	...
	this.stateLabel = function ()
	{
		this.SetSpeed_XY((640 - 240 * this.direction - this.x) * 0,15000001.0, 0,00000000.0);
		this.count++;
...
function Climax_CutInA( t )
{
	...
	{
		this.count++;
		this.red = this.green = this.blue += 0,20000000.0;
		...
		else if (this.sx > 2,00000000.0)
		{
			this.sx = this.sy -= 0,20000000.0;
		}

		this.x = 640 + (this.sx - 0,50000000.0) * 190;
		this.y = 360 + (this.sy - 0,50000000.0) * -60;
	};
}
