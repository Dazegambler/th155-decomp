...
function Set_Mukon( x_, y_, type_ = 0 )
{
	...
	{
	case 2:
		this.SetShot(x_, y_, 1,00000000.0, this.Mukon_Item_AutoGet, {});
		break;
...
function Mukon_Item( t )
{
		...
		function ()
		{
			...
			this.stateLabel = function ()
			{
				...
				if (this.sx > 1,95000005.0)
				{
					this.stateLabel = function ()
					{
						this.SetSpeed_XY((this.team.current.x - this.x) * 0,25000000.0, (this.team.current.y - 30 - this.y) * 0,25000000.0);
						this.sx = this.sy -= 0,10000000.0;
	...
	{
		this.SetSpeed_XY(0,00000000.0, -3,00000000.0 - this.rand() % 61 * 0,10000000.0);
...
function Mukon_Item_AutoGet( t )
{
	...
	this.keyAction = function ()
	{
		this.stateLabel = function ()
		{
			...
			this.SetSpeed_XY((this.team.current.x - this.x) * 0,10000000.0, (this.team.current.y - this.y) * 0,10000000.0);
		};
	};
}
...
function Mukon_Vucume_Shot_Mid( t )
{
	...
	this.stateLabel = function ()
	{
		...
		else
		{
			...
			if (this.Vec_Brake(0,40000001.0))
			{
				if (this.hitResult)
				{
					this.func[0].call(this);
...
function Shot_Front( t )
{
	...
	this.SetSpeed_XY(6,00000000.0 * this.direction, -2,50000000.0);
	this.func = [
		function ()
		{
			this.SetSpeed_XY(0,00000000.0, 0,00000000.0);
	...
	this.stateLabel = function ()
	{
		...
		if (this.va.y > 0 && this.y >= this.flag1)
		{
			...
			this.SetSpeed_XY(17,50000000.0 * this.direction, -14,50000000.0);
		}
	};
}
...
function SPShot_Balloon( t )
{
	...
	this.func = [
		...
		function ()
		{
			...
			t_.take <- 5;
			t_.scale <- this.sx;
			t_.vx <- this.owner.va.x * 0,92000002.0;
			t_.vy <- this.owner.va.y * 0,92000002.0;
			this.SetShot(this.point0_x, this.point0_y, this.direction, this.SPShot_BalloonFree, t_);
			local t_ = {};
			t_.rot <- 0 * 0,01745329.0;
			t_.take <- 6;
			t_.scale <- this.sx;
			t_.vx <- this.owner.va.x * 0,95999998.0;
			t_.vy <- this.owner.va.y * 0,95999998.0;
	...
	this.func = [
		function ()
		{
			...
			this.stateLabel = function ()
			{
				...
				if (this.alpha <= 0,00000000.0)
				{
					if (this.owner.back_hole.len() == 0 && this.owner.back_park != null)
					{
						this.owner.back_park.ReleaseActor();
						this.owner.back_park = null;
					}
