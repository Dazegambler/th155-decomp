...
function BeginBattleB_Object( t )
{
	...
	this.SetSpeed_XY(20,00000000.0 * this.direction, -10,00000000.0);
	...
	this.keyAction = [
		...
		function ()
		{
			...
			{
				this.VX_Brake(this.va.x * this.direction >= 5,00000000.0 ? 0,20000000.0 : 0,00000000.0);
				this.AddSpeed_XY(0,00000000.0, 1,39999998.0);
				this.count++;
	...
	this.subState = function ()
	{
		this.VX_Brake(this.va.x * this.direction >= 5,00000000.0 ? 0,40000001.0 : 0,00000000.0);
		this.AddSpeed_XY(0,00000000.0, 0,40000001.0);

		if (this.x > ::battle.corner_right - 50 && this.direction == 1,00000000.0 || this.x < ::battle.corner_left + 50 && this.direction == -1,00000000.0)
		{
...
function Shot_Normal( t )
{
	...
	this.func = [
		function ()
		{
			...
			if (this.flag1)
			{
				...
				this.SetSpeed_XY.call(this.flag1, this.va.x * 0,50000000.0, this.va.y * 0,50000000.0);
			}

			this.stateLabel = function ()
			{
				this.sx = this.sy *= 0,89999998.0;
				this.alpha = this.red = this.green -= 0,10000000.0;
		...
		function ()
		{
			...
			this.stateLabel = function ()
			{
				if (this.IsScreen(100,00000000.0) || this.cancelCount <= 0 || this.hitCount > 0 || this.grazeCount > 0 || this.Damage_ConvertOP(this.x, this.y, 4))
				{
					this.func[0].call(this);
					return true;
				}

				this.AddSpeed_Vec(0,75000000.0, this.rz, 15,00000000.0, this.direction);
			};
			this.PlaySE(3080);
	...
	this.SetSpeed_Vec(3,50000000.0, t.rot, this.direction);
...
function Shot_NormalMini( t )
{
	...
	this.flag2 = this.Vector3();
	this.flag2.x = 0,25000000.0;
	this.flag2.RotateByRadian(t.rot2);
	this.flag2.y *= 2,50000000.0;
	...
	this.flag2.x = this.va.x * 0,10000000.0;
	this.flag2.y = this.va.y * 0,10000000.0;
	this.keyAction = function ()
	{
		this.ReleaseActor();
	};
	...
	this.stateLabel = function ()
	{
		if (this.IsScreen(100,00000000.0) || this.cancelCount <= 0 || this.hitCount > 0 || this.grazeCount > 0 || this.Damage_ConvertOP(this.x, this.y, 2))
		{
			this.func();
			return true;
		}
...
function Shot_Normal_Aura( t )
{
	...
	this.func = [
		...
		function ()
		{
			this.stateLabel = function ()
			{
				...
				this.anime.top -= 10,00000000.0;
				this.sx = this.sy += (1,00000000.0 - this.sx) * 0,15000001.0;
			};
		}
	];
	this.anime.left = 0;
	...
	this.anime.vertex_red1 = 0,00000000.0;
	this.anime.vertex_green1 = 0,00000000.0;
	this.ry = -75 * 0,01745329.0;
	this.sx = this.sy = 0,60000002.0;
	this.stateLabel = function ()
	{
		this.rz -= 3,00000000.0 * 0,01745329.0;
		this.anime.top -= 5,00000000.0;
	};
}
...
function Shot_Front( t )
{
	...
	this.stateLabel = function ()
	{
		...
		if (this.direction == 1,00000000.0 && this.x > ::battle.corner_right || this.direction == -1,00000000.0 && this.x < ::battle.corner_left)
		{
			this.VX_Brake(1,50000000.0);
		}

		this.sx += 0,01000000.0;
		this.SetCollisionScaling(this.sx, this.sy, 1,00000000.0);
...
function Shot_FrontVortexA( t )
{
	...
	this.rx = (10 - this.rand() % 21) * 0,01745329.0;
	this.ry = 70 * 0,01745329.0;
	this.flag1 = 6,00000000.0 + this.rand() % 3;
	this.func = [
		function ()
		{
			this.stateLabel = function ()
			{
				...
				this.rz += 24,00000000.0 * 0,01745329.0;
				this.anime.radius0 += 4,40000010.0;
				this.anime.radius1 += (this.anime.radius0 - this.anime.radius1) * 0,10000000.0;
				this.anime.length *= 0,94999999.0;
				this.alpha -= 0,05000000.0;
				this.red = this.green = this.blue -= 0,00500000.0;
	...
	this.stateLabel = function ()
	{
		...
		this.VX_Brake(this.va.x * this.direction >= 2,00000000.0 ? 0,50000000.0 : 0,01000000.0);
		this.rz += 18,00000000.0 * 0,01745329.0;
		this.anime.radius0 += this.flag1;
		this.anime.radius1 += this.flag1 * 1,25000000.0;
		this.flag1 *= 0,85000002.0;
		this.anime.length += (96 - this.anime.length) * 0,10000000.0;
		this.count++;
...
function Shot_FrontVortexB( t )
{
	...
	this.rx = (10 - this.rand() % 21) * 0,01745329.0;
	this.ry = 70 * 0,01745329.0;
	this.flag1 = 1,00000000.0 + this.rand() % 2;
	this.func = [
		function ()
		{
			this.stateLabel = function ()
			{
				...
				{
					this.SetSpeed_XY((this.initTable.target.x + 128 * this.direction - this.x) * 0,05000000.0, 0,00000000.0);
				}
				else
				{
					this.VX_Brake(1,50000000.0);
				...
				this.rz += 24,00000000.0 * 0,01745329.0;
				this.anime.radius0 += 12,39999962.0;
				this.anime.radius1 += (this.anime.radius0 - this.anime.radius1) * 0,10000000.0;
				this.anime.length *= 0,85000002.0;
				this.alpha -= 0,10000000.0;
				this.red = this.green = this.blue -= 0,00500000.0;
	...
	this.stateLabel = function ()
	{
		...
		this.rz += 40,00000000.0 * 0,01745329.0;
		this.anime.radius0 += this.flag1;
		this.anime.radius1 += this.flag1 * 1,25000000.0;
		this.flag1 *= 0,85000002.0;
		this.anime.length += (160 - this.anime.length) * 0,10000000.0;
...
function Shot_Charge( t )
{
	...
	this.flag1.x = this.rand() % 11 * 0,10000000.0;
	this.flag1.y = this.rand() % 11 * 0,10000000.0;
	this.flag1.z = this.rand() % 11 * 0,10000000.0;
	this.count = 15;
	this.stateLabel = function ()
	{
		...
		local s_ = 0,00000000.0;

		if (this.flag2 < 30)
		{
			s_ = (0,50000000.0 - this.sx) * 0,10000000.0;
		}
		else
		{
			s_ = (1,50000000.0 - this.sx) * 0,50000000.0;
		}
		...
		if (this.flag3)
		{
			...
			this.flag3.SetSpeed_XY((1,00000000.0 - this.sx) * 15 * this.direction, (1,00000000.0 - this.sy) * 15);
		}
	};
	this.func = [
		function ()
		...
		function ()
		{
			...
			this.stateLabel = function ()
			{
				this.sx = this.sy += (0,25000000.0 - this.sx) * 0,50000000.0;

				if (this.IsScreen(100,00000000.0))
				{
					this.ReleaseActor();
					return;
				}
		...
		function ()
		{
			...
			if (this.initTable.charge)
			{
				...
				this.stateLabel = function ()
				{
					this.sx = this.sy += (6,00000000.0 - this.sx) * 0,20000000.0;
					this.SetCollisionScaling(this.sx, this.sy, 1,00000000.0);
					this.count++;
						...
						this.stateLabel = function ()
						{
							...
							if (this.count % 5 == 1)
							{
								for( local i = 0; i < 3; i++ )
								{
									local t_ = {};
									t_.v <- 2,00000000.0;
									t_.rot <- this.count * 5 * 0,01745329.0 + i * 120 * 0,01745329.0;
...
function Shot_ChargeMarker( t )
{
	...
	this.stateLabel = function ()
	{
		this.rz -= 0,25000000.0 * 0,01745329.0;
		this.count++;
...
function Shot_ChargeFire( t )
{
	...
	this.SetCollisionScaling(3,00000000.0, 2,00000000.0, 1,00000000.0);
	this.stateLabel = function ()
	{
		this.sx += (4,00000000.0 - this.sx) * 0,25000000.0;
		this.sy *= 0,89999998.0;
		this.alpha -= 0,10000000.0;
...
function Shot_ChargeParticle( t )
{
	...
	this.flag1 = 0,10000000.0 + this.rand() % 11 * 0,01000000.0;
	...
	this.stateLabel = function ()
	{
		...
		if (this.sx >= this.flag1 - 0,05000000.0)
		{
			this.stateLabel = function ()
			{
				this.sx = this.sy *= 0,97000003.0;
				this.flag2 += (1,00000000.0 - this.flag2) * 0,05000000.0;
				this.flag3.x = (this.initTable.pare.x - this.x) * 0,25000000.0;
				this.flag3.y = (this.initTable.pare.y - this.y) * 0,25000000.0;
...
function Shot_ChargeExp( t )
{
	...
	this.stateLabel = function ()
	{
		this.sx = this.sy += (6,00000000.0 - this.sx) * 0,25000000.0;
...
function Shot_Barrage_Core( t )
{
	...
	this.subState = function ()
	{
		this.flag1.RotateByRadian(-0,08726646.0);
		this.vec.x = this.flag1.x;
		this.vec.y = this.flag1.y;
		this.vec.y *= 0,33000001.0;
...
function Shot_Okult( t )
{
	...
	this.flag2 = this.Vector3();
	this.flag3 = null;
	this.flag4 = 1,25000000.0;
	this.SetSpeed_XY(14,50000000.0 * this.direction * this.flag4, 0,00000000.0);
	this.func = [
		function ()
		{
		...
		function ()
		{
			...
			this.subState = function ()
			{
				if (this.Vec_Brake(3,00000000.0))
				{
					this.count++;
					...
					if (this.count >= 10)
					{
						this.subState = function ()
						{
							...
							this.flag2.SetLength(3,00000000.0);
							...
							this.rz = this.atan2(this.va.y, this.va.x * this.direction) + 3,14159203.0;
						};
					}
				}
			};
		...
		function ()
		{
			...
			this.subState = function ()
			{
				if (this.Vec_Brake(6,00000000.0))
				{
					this.subState = function ()
					{
						this.flag2.x = this.owner.point0_x - this.x;
	...
	this.stateLabel = function ()
	{
		...
		{
			this.rz -= 2,00000000.0 * 0,01745329.0 * this.flag4;
			this.va.RotateByDegree(-2,00000000.0 * this.direction * this.flag4);
		}
	...
	this.func = [
		...
		function ()
		{
			...
			this.stateLabel = function ()
			{
				...
				this.Vec_Brake(2,00000000.0, 0,00000000.0);
...
function SPShot_A_TigerGate( t )
{
	...
	this.stateLabel = function ()
	{
		this.sx = this.sy += (1,50000000.0 - this.sx) * 0,25000000.0;
...
function SPShot_B_Feather( t )
{
	...
	this.rz = this.rand() % 360 * 0,01745329.0;
	this.flag1 = (5,00000000.0 + this.rand() % 6) * 0,01745329.0;
	this.SetSpeed_XY(3,00000000.0 - this.rand() % 7, -this.rand() % 4);
	this.sx = this.sy = 0,89999998.0 + this.rand() % 3 * 0,10000000.0;
	this.stateLabel = function ()
	{
		this.Vec_Brake(0,02500000.0, 1,00000000.0);
...
function SPShot_B_AuraF( t )
{
	...
	this.func = [
		function ()
		{
			...
			this.stateLabel = function ()
			{
				this.sx *= 1,04999995.0;
				this.sy *= 0,89999998.0;
				this.alpha = this.red = this.green -= 0,15000001.0;
...
function SPShot_B_Aura( t )
{
	...
	this.stateLabel = function ()
	{
		this.sx += (2,50000000.0 - this.sx) * 0,10000000.0;
...
function SP_C_SparkCharge( t )
{
	...
	this.stateLabel = function ()
	{
		this.anime.radius0 += (this.initTable.scale - this.anime.radius0) * 0,25000000.0;
		this.anime.radius1 += (this.initTable.scale - this.anime.radius1) * 0,25000000.0;
...
function SPShot_D_OpenSplash( t )
{
	...
	this.stateLabel = function ()
	{
		...
		local s_ = (1,75000000.0 - this.sx) * 0,20000000.0;
		...
		this.sy *= this.flag1;
		this.count++;
		this.AddSpeed_XY(0,00000000.0, 0,60000002.0);
...
function SPShot_D_OpenSplashB( t )
{
	...
	this.stateLabel = function ()
	{
		local s_ = (1,10000002.0 - this.sx) * 0,20000000.0;
...
function SPShot_D_Edge( t )
{
	...
	this.DrawActorPriority(179);
	this.rz = (-5 + this.rand() % 11) * 0,01745329.0;
	this.alpha = 0,00000000.0;
	this.flag1 = 1,00000000.0;
	...
	this.func = [
		function ()
		{
			this.subState = function ()
			{
				this.alpha = this.red = this.green -= 0,02500000.0;
...
function SPShot_D( t )
{
	...
	this.func = [
		function ()
		{
			...
			this.stateLabel = function ()
			{
				this.sx = this.sy += -this.sx * 0,20000000.0;
				this.alpha -= 0,05000000.0;
...
function SPShot_D_Ball( t )
{
	...
	this.SetSpeed_XY(4,00000000.0 * this.direction, -18,00000000.0);
	this.cancelCount = 3;
	this.atk_id = 8388608;
	...
	this.func = [
		function ()
		{
			this.PlaySE(3068);
			this.callbackGroup = 0;
			this.SetSpeed_XY(-5,00000000.0 * this.direction, -6,00000000.0);
			this.owner.ball = null;
			this.stateLabel = function ()
			{
				this.alpha -= 0,05000000.0;
				this.sx = this.sy += (1,33000004.0 - this.sx) * 0,10000000.0;
	...
	this.stateLabel = function ()
	{
		...
		this.rz -= 6,00000000.0 * 0,01745329.0;
		this.AddSpeed_XY(0,00000000.0, this.va.y < 0,00000000.0 ? 0,66000003.0 : 0,25000000.0);

		if (this.keyTake == 0 && this.va.y > 0,00000000.0)
		{
			this.SetMotion(this.motion, 1);
		}
	};
...
function SPShot_E( t )
{
	...
	this.flag2 = 0;
	this.SetSpeed_Vec(27,50000000.0, t.rot, this.direction);
	this.func = [
		function ()
		{
			this.PlaySE(3048);
			this.callbackGroup = 0;
			this.count = 0;
			this.SetSpeed_XY(0,00000000.0, -3,00000000.0);

			for( local i = 0; i < 360; i = i + (30 + this.rand() % 10) )
			{
				local t_ = {};
				t_.rot <- i * 0,01745329.0;
			...
			for( local i = 0; i < 2; i++ )
			{
				this.SetFreeObject(this.x, this.y, 1,00000000.0 - 2,00000000.0 * i, function ( t )
				{
					this.SetMotion(3049, 2);
					this.SetSpeed_XY(0,00000000.0, -3,00000000.0);
					this.rx = (45 + this.rand() % 30) * 0,01745329.0 * this.direction;
					this.ry = (45 + this.rand() % 30) * 0,01745329.0;
					this.flag1 = 2,20000005.0 + this.rand() % 12 * 0,10000000.0;
					this.stateLabel = function ()
					{
						this.sx = this.sy += (this.flag1 - this.sx) * 0,15000001.0;
						this.AddSpeed_XY(0,00000000.0, 0,30000001.0);
						this.alpha = this.red = this.green -= 0,07500000.0;
...
function SPShot_E_SprashShot( t )
{
	...
	this.SetSpeed_Vec(8,00000000.0, t.rot, 1,00000000.0);
	this.SetSpeed_XY(null, this.va.y * 0,25000000.0 - 8,00000000.0);
	this.va.RotateByRadian(30 * 0,01745329.0);
	this.SetSpeed_XY(this.va.x * this.direction, null);
	this.AddSpeed_XY(8,00000000.0 * this.direction * 0,86600000.0, -8,00000000.0 * 0,50000000.0);
	this.cancelCount = 1;
	this.atk_id = 16777216;
	this.func = [
		function ()
		{
			...
			this.stateLabel = function ()
			{
				...
				this.alpha = this.red = this.green -= 0,20000000.0;
	...
	this.stateLabel = function ()
	{
		...
		this.SetSpeed_XY(this.va.x * 0,97500002.0, this.va.y + 0,44999999.0);
...
function SPShot_E_RingSplash( t )
{
	...
	this.stateLabel = function ()
	{
		local s_ = (1,50000000.0 - this.sx) * 0,12500000.0;
...
function SPShot_E_Splash( t )
{
	...
	this.SetSpeed_XY(null, this.va.y * 0,33000001.0);
	this.stateLabel = function ()
	{
		this.AddSpeed_XY(0,00000000.0, 0,50000000.0);
		this.sx = this.sy += 0,05000000.0;
		this.alpha = this.red = this.green -= 0,03300000.0;
...
function Spell_A_Net( t )
{
	...
	this.anime.vertex_alpha1 = 0,00000000.0;
	this.anime.vertex_blue1 = 0,80000001.0;
	this.anime.vertex_red1 = 0,80000001.0;
	this.anime.vertex_green1 = 0,80000001.0;
	this.flag1 = ::manbow.Actor2DProcGroup();
	this.flag2 = 0,10000000.0;
	this.anime.radius1 = 128 * this.flag2;
	this.anime.length = 255 * this.flag2;
	this.SetCollisionRotation(0,00000000.0, 0,00000000.0, 45 * 0,01745329.0);
	this.SetCollisionScaling(this.flag2, this.flag2, 1,00000000.0);
	this.func = [
		function ()
		{
			this.callbackGroup = 0;
			...
			this.stateLabel = function ()
			{
				...
				this.alpha -= 0,25000000.0;
				this.anime.top += 15;
				this.anime.radius0 *= 0,94999999.0;
				this.anime.radius1 *= 0,94999999.0;
	...
	this.stateLabel = function ()
	{
		...
		this.flag2 += (4,00000000.0 - this.flag2) * 0,10000000.0;
		this.anime.radius1 = 128 * this.flag2;
		this.anime.length = 255 * this.flag2;
		this.anime.top -= 10;
...
function Spell_A_NetHand( t )
{
	...
	this.ry = (50 + this.rand() % 41) * 0,01745329.0;
	this.rx -= (40 + this.rand() % 11) * 0,01745329.0 * this.direction;
	this.SetSpeed_Vec(1,50000000.0 + this.rand() % 16 * 0,10000000.0, 45 * 0,01745329.0, this.direction);
	this.sx = this.sy = 0,05000000.0;
	this.flag1 = 1,00000000.0 + this.rand() % 5 * 0,10000000.0;
	this.stateLabel = function ()
	{
		this.SetSpeed_XY(this.va.x * 0,97000003.0, this.va.y * 0,97000003.0);
		this.rz += -18 * 0,01745329.0;
		this.sx = this.sy += (this.flag1 - this.sx) * 0,20000000.0;
		this.alpha -= 0,05000000.0;
...
function Spell_A_NetB( t )
{
	...
	this.rz = this.rand() % 360 * 0,01745329.0;
	this.ry = (50 + this.rand() % 41) * 0,01745329.0;
	this.rx -= 45 * 0,01745329.0 * this.direction;
	this.flag1 = 0,25000000.0 + this.rand() % 75 * 0,01000000.0;
	this.flag2 = (15,00000000.0 + this.rand() % 120 * 0,10000000.0) * 0,01745329.0;
	...
	this.func = [
		function ()
		{
			...
			{
				this.flag1 += 0,10000000.0;
				this.rz += this.flag2 * 1,75000000.0;
				this.alpha -= 0,05000000.0;
	...
	this.SetSpeed_Vec(16,00000000.0 + this.rand() % 4, 45 * 0,01745329.0, this.direction);
	this.stateLabel = function ()
	{
		this.SetSpeed_XY(this.va.x * 0,97000003.0, this.va.y * 0,97000003.0);
		this.flag1 += 0,10000000.0;
...
function Spell_A_CapNet( t )
{
	...
	this.alpha = 0,00000000.0;
	this.sx = this.sy = 1,50000000.0 + this.rand() % 16 * 0,10000000.0;
	this.SetParent(this.owner.target, this.owner.target.x - this.x, this.owner.target.y - this.y);
	this.stateLabel = function ()
	{
		this.rz += 16,00000000.0 * 0,01745329.0;
		this.sx = this.sy *= 0,66000003.0;
		this.alpha += 0,33000001.0;

		if (this.sx <= 0,10000000.0)
		{
			this.ReleaseActor();
		}
	};
...
function Spell_A_HitSpinEffect( t )
{
	...
	this.ry = 45 * 0,01745329.0;
	this.rx -= 50 * 0,01745329.0 * this.direction;
	this.sx = this.sy = 1,50000000.0 + this.rand() % 21 * 0,01745329.0;
	this.stateLabel = function ()
	{
		this.SetSpeed_XY(this.va.x * 0,97000003.0, this.va.y * 0,97000003.0);
		this.rz += -18 * 0,01745329.0;
...
function SpellShot_B( t )
{
	...
	this.stateLabel = function ()
	{
		...
		this.sx = this.sy += (3,50000000.0 - this.sx) * 0,25000000.0;
		this.SetCollisionScaling(this.sx, this.sy, 1,00000000.0);
		this.count++;
...
function SpellShot_B_Shock( t )
{
	...
	this.stateLabel = function ()
	{
		this.sx = this.sy += (4,00000000.0 - this.sx) * 0,25000000.0;

		if (this.sx > 3,75000000.0)
		{
			this.stateLabel = function ()
			{
				this.sy *= 0,80000001.0;
...
function SpellShot_B_AuraB( t )
{
	...
	this.func = function ()
	{
		this.stateLabel = function ()
		{
			this.rz -= 2,50000000.0 * 0,01745329.0;
			this.sx = this.sy += 0,01000000.0;
			this.alpha = this.green = this.red -= 0,05000000.0;
	...
	this.subState = function ()
	{
		...
		if (this.alpha >= 1,00000000.0)
		{
			this.subState = function ()
			{
				this.alpha = this.green = this.red -= 0,02500000.0;
			};
		}
	};
	this.stateLabel = function ()
...
function SpellShot_C_Splash( t )
{
	...
	this.stateLabel = function ()
	{
		this.flag1 = (3,50000000.0 - this.sx) * 0,10000000.0;

		if (this.flag1 < 0,01500000.0)
		{
			this.flag1 = 0,01500000.0;
		}
...
function SpellC_Marker( t )
{
	...
	for( local i = 0; i < 3; i++ )
	{
		...
		pos_.RotateByRadian(120 * 0,01745329.0);
	}
	...
	this.stateLabel = function ()
	{
		...
		{
			this.x += (a.x - (this.x + this.initTable.pos.x * this.direction)) * 0,10000000.0;
			this.y += (a.y - (this.y + this.initTable.pos.y)) * 0,10000000.0;
		});
		this.rz -= 0,25000000.0 * 0,01745329.0;
		this.count++;
...
function SpellC_MarkerCount( t )
{
	...
	this.func = [
		function ()
		{
			...
			this.stateLabel = function ()
			{
				this.initTable.pos.RotateByDegree(2,00000000.0);
				this.sx = this.sy *= 0,80000001.0;
...
function Climax_VortexA( t )
{
	...
	this.alpha = 0,00000000.0;
	this.rz = this.rand() % 360 * 0,01745329.0;
	this.rx = (70 + this.rand() % 5) * 0,01745329.0;
	this.ry = (-25 - this.rand() % 25) * 0,01745329.0 * this.direction;
	this.func = [
		function ()
		{
			this.stateLabel = function ()
			{
				this.AddSpeed_XY(0,00000000.0, -0,10000000.0);
				this.rz += 24,00000000.0 * 0,01745329.0;
				this.anime.radius0 += 1,39999998.0;
				this.anime.radius1 += (this.anime.radius0 - this.anime.radius1) * 0,10000000.0;
				this.anime.length *= 0,50000000.0;
				this.alpha -= 0,05000000.0;
				this.red = this.green = this.blue -= 0,00500000.0;
	...
	this.stateLabel = function ()
	{
		...
		this.anime.radius0 += 40,00000000.0;
		this.anime.radius1 += 1,10000002.0;
		this.anime.length += (128 - this.anime.length) * 0,10000000.0;
		this.count++;
...
function Climax_VortexB( t )
{
	...
	this.anime.length = 64;
	this.rz = this.rand() % 360 * 0,01745329.0;
	this.rx = -70 * 0,01745329.0 * this.direction;
	this.ry = 60 * 0,01745329.0;
	this.func = [
		function ()
		{
			this.stateLabel = function ()
			{
				this.rz -= 12,00000000.0 * 0,01745329.0;
				this.anime.height += (256 - this.anime.height) * 0,05000000.0;
				this.anime.radius0 *= 0,98000002.0;
				this.anime.radius1 *= 0,97000003.0;
				this.anime.length *= 1,04999995.0;
				this.alpha -= 0,10000000.0;
	...
	this.stateLabel = function ()
	{
		...
		this.anime.height += (256 - this.anime.height) * 0,05000000.0;
		this.anime.radius0 *= 0,98000002.0;
		this.anime.radius1 *= 0,98000002.0;
		this.anime.length *= 0,99000001.0;
		this.alpha -= 0,02500000.0;
...
function Climax_VortexC( t )
{
	...
	this.func = [
		...
		function ()
		{
			...
			this.stateLabel = function ()
			{
				this.sx = this.sy += (2,00000000.0 - this.sx) * 0,15000001.0;
			};
		}
	];
	this.stateLabel = function ()
...
function Climax_Arm( t )
{
	...
	this.stateLabel = function ()
	{
		this.AddSpeed_XY(0,00000000.0, this.va.y <= 0,25000000.0 ? 0,50000000.0 : 0,00500000.0);
		local r_ = -this.rz * 0,10000000.0;
		r_ = this.Math_MinMax(r_, 23,00000000.0 * 0,01745329.0, 35,00000000.0 * 0,01745329.0);
		this.rz += r_;
	};
}
...
function Climax_FaceB( t )
{
	...
	this.func = [
		...
		{
			this.PlaySE(3055);
			this.flag4 = this.SetFreeObject(this.point0_x, this.point0_y, 1,00000000.0, this.Climax_FaceB_Light, {}).weakref();
...
function Climax_FaceB_LightBar( t )
{
	...
	this.sy = 0,75000000.0 + this.rand() % 5 * 0,10000000.0;
	this.rz = t.rot;
	this.func = [
		function ()
		{
