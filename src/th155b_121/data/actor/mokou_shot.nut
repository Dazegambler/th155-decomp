...
function Shot_FrontB( t )
{
	...
	else
	{
		...
		this.flag1 = 0,97000003.0;
	}

	this.rz = t.rot;
	this.atk_id = 65536;
	this.SetSpeed_Vec(15 + this.cos(this.rz * 2) * 10,00000000.0, this.rz, this.direction);
	this.func = [
		function ()
		{
			this.callbackGroup = 0;
	...
	this.stateLabel = function ()
	{
		...
		this.SetSpeed_XY(this.va.x, this.va.y * 0,94999999.0);
...
function Shot_FrontB2( t )
{
	...
	this.SetSpeed_XY(0,00000000.0, -t.v);
	this.flag1 = this.y;
	this.stateLabel = function ()
	{
		this.va.RotateByRadian(6,00000000.0 * 0,01745329.0);
	...
	this.func = [
		...
		function ()
		{
			...
			if (this.initTable.charge)
			{
				...
				{
					local t_ = {};
					t_.v <- 40,00000000.0 + this.rand() % 11;
					t_.rot <- (-40 - this.rand() % 20) * 0,01745329.0;
					t_.wait <- 40 + i * 5;
			...
			else
			{
				...
				{
					local t_ = {};
					t_.v <- 20,00000000.0 + this.rand() % 6;
					t_.rot <- (-50 - this.rand() % 25) * 0,01745329.0;
					t_.addV <- 0,00000000.0;
					t_.occult <- this.initTable.occult;

					if (this.initTable.k < 0)
					{
						t_.addV = 6,00000000.0;
					}
...
function Shot_Charge_FirePart( t )
{
	...
	this.sx = 0,50000000.0 + this.rand() % 25 * 0,10000000.0;
	this.sy = 0,50000000.0 + this.rand() % 25 * 0,10000000.0;
	this.keyAction = this.ReleaseActor;
	this.SetSpeed_Vec(5,00000000.0 + this.rand() % 6, this.rz, this.direction);
	this.stateLabel = function ()
	{
...
function Shot_ChargeB( t )
{
	...
	this.SetSpeed_Vec(t.v, t.rot, this.direction);
	this.AddSpeed_XY(t.addV * this.direction, 0,00000000.0);
	...
	this.func = [
		function ()
		{
			...
			this.stateLabel = function ()
			{
				...
				if (this.flag1)
				{
					this.flag1.alpha -= 0,25000000.0;
					this.flag1.anime.length *= 0,80000001.0;
					this.flag1.anime.radius0 *= 0,80000001.0;

					if (this.flag1.alpha <= 0,00000000.0)
					{
	...
	this.stateLabel = function ()
	{
		if (this.va.y > 5,00000000.0 && this.keyTake == 3)
		{
		...
		this.AddSpeed_XY(0,00000000.0, 0,75000000.0 + 0,25000000.0);
	};
}
...
function Shot_Change( t )
{
	...
	{
		if (this.Vec_Brake(0,05000000.0, 5,00000000.0))
		{
			this.subState = function ()
			{
			};
...
function SPShot_B( t )
{
	...
	this.stateLabel = function ()
	{
		...
		this.rz -= 8,50000000.0 * 0,01745329.0;
		this.AddSpeed_XY(0,00000000.0, this.va.y < 0,50000000.0 ? 0,50000000.0 : 0,05000000.0);
		this.count++;

		if (this.count >= 10)
		{
			this.red = this.green = this.blue += 0,15000001.0;
		}
...
function SPShot_D( t )
{
	...
	this.SetFreeObject(this.x, this.y, this.direction, function ( b_ )
	{
		...
		this.stateLabel = function ()
		{
			this.sx = this.sy += (5,00000000.0 - this.sx) * 0,25000000.0;
			this.alpha -= 0,05000000.0;
...
{
	this.SetMotion(3039, 1);
	this.SetSpeed_Vec(11,00000000.0, 45 * 0,01745329.0, this.direction);
	this.stateLabel = function ()
	{
		this.count++;
...
function SPShot_D_Wing( t )
{
	...
	this.stateLabel = function ()
	{
		...
		this.rz += (-80 * 0,01745329.0 - this.rz) * 0,02500000.0;
...
function SPShot_E_Vortex( t )
{
	...
	this.anime.length = 0;
	this.ry = 0,01745329.0 * this.rand() % 360;
	...
	this.func = [
		function ()
		{
			this.stateLabel = function ()
			{
				this.ry += 16,00000000.0 * 0,01745329.0;
				this.AddSpeed_XY(0,00000000.0, -0,75000000.0);
				this.alpha = this.blue = this.green -= 0,07500000.0;
	...
	this.stateLabel = function ()
	{
		...
		this.anime.length += (48 - this.anime.length) * 0,05000000.0;
...
function SPShot_E_ResFireDyna( t )
{
	...
	this.func = [
		function ()
		{
			...
			this.stateLabel = function ()
			{
				...
				this.anime.radius1 += 18,00000000.0;
	...
	this.stateLabel = function ()
	{
		...
		this.anime.radius1 *= 0,94000000.0;
	};
}
...
function SPShot_E_ResFire( t )
{
	...
	this.rx = (-30 + this.rand() % 61) * 0,01745329.0;
	this.SetParent(this.owner, 0, 0);
	this.DrawActorPriority(t.priority);
	this.alpha = 0,00000000.0;
	this.flag1 = (2 + this.rand() % 3) * 0,10000000.0;
	this.func = [
		function ()
		{
			this.SetParent(null, 0, 0);
	...
	this.stateLabel = function ()
	{
		this.rz -= 8,80000019.0 * 0,01745329.0;
...
function SPShot_E_FirePilar( t )
{
	if (t.occult)
	{
		...
		this.stateLabel = function ()
		{
			...
			this.SetCollisionScaling(this.flag1, this.sy, 1,00000000.0);
			this.SetSpeed_XY(0,00000000.0, (::battle.scroll_bottom + 50 - this.y) * 0,10000000.0);
			this.alpha -= 0,10000000.0;
	...
	this.sx = (1,00000000.0 + 0,25000000.0 * t.level) * (1,00000000.0 + this.rand() % 31 * 0,01000000.0);
	this.sy = 2,00000000.0;
	this.flag1 = 1,00000000.0 + 0,25000000.0 * t.level;
	this.SetParent(this.owner, this.x - this.owner.x, this.y - this.owner.y);
	this.SetCollisionScaling(this.flag1, this.sy, 1,00000000.0);
	this.alpha = 3,00000000.0;
	this.atkRate_Pat = 1,00000000.0 + t.level * 0,25000000.0;
}
...
function SPShot_E_FirePart( t )
{
	if (t.occult)
	{
		...
		this.sy = 0,50000000.0 + this.rand() % 15 * 0,10000000.0;
		this.stateLabel = function ()
		{
			this.AddSpeed_XY(0,00000000.0, -0,75000000.0);
			this.sx += 0,02500000.0;
			this.sy += 0,16000000.0;
...
function SpellShotA_VortexA( t )
{
	if (t.occult)
	{
		...
		this.stateLabel = function ()
		{
			this.ry += 12 * 0,01745329.0;
			this.flag1 += 0,02500000.0;
			this.anime.radius1 += 10,00000000.0;
			this.anime.radius0 += 8,00000000.0;
			this.AddSpeed_XY(0,00000000.0, -0,85000002.0);

			if (this.flag1 >= 0,69999999.0)
			{
			...
			{
				this.alpha = this.green = this.red += 0,15000001.0;
	...
	else
	{
		...
		this.alpha = this.green = this.blue = 0,00000000.0;
		this.stateLabel = function ()
		{
			this.ry += 12 * 0,01745329.0;
			this.flag1 += 0,02500000.0;
			...
			{
				this.alpha = this.green = this.blue += 0,15000001.0;

				if (this.alpha > 1,00000000.0)
				{
					this.alpha = this.green = this.blue = 1,00000000.0;
				}
			}
		};
	}
	...
	this.func = [
		function ()
		{
			...
			{
				this.ry += 12 * 0,01745329.0;
				this.anime.length *= 0,92500001.0;
				this.anime.radius0 += (this.anime.radius1 - this.anime.radius0) * 0,20000000.0;
				this.alpha -= 0,10000000.0;
...
function SpellShotA_FirePilar( t )
{
	...
	this.DrawActorPriority(199);
	this.sx = 1,00000000.0 + this.rand() % 31 * 0,01000000.0;
	this.sy = 2,00000000.0;
	this.SetParent(this.owner, this.x - this.owner.x, this.y - this.owner.y);
	this.alpha = 3,00000000.0;
	this.stateLabel = function ()
...
function Spell_B_Fiji( t )
{
	...
	this.stateLabel = function ()
	{
		...
		if (this.count % 10 == 1)
		{
			...
			t_.occult <- this.initTable.occult;
			this.flag3.Add(this.SetShot(this.x, ::battle.scroll_bottom + 1024, 1,00000000.0 - this.rand() % 2 * 2, this.Spell_B_FirePilar, t_));
		}
...
function Spell_B_FirePilar( t )
{
	...
	this.sx = this.flag1 + this.rand() % 31 * 0,01000000.0;
	this.sy = 2,00000000.0;
	this.alpha = 3,00000000.0;
	this.cancelCount = 3;
...
function Spell_C_Amulet( t )
{
	...
	this.rz = t.rot;
	this.SetSpeed_Vec(60,00000000.0, this.rz, this.direction);
	this.atkRate_Pat = t.rate;
	this.atk_id = 67108864;
	this.SetAllHitShot();
	this.func = [
		...
		function ()
		{
			...
			this.stateLabel = function ()
			{
				this.sx = this.sy += (1,33000004.0 - this.sx) * 0,25000000.0;
	...
	this.stateLabel = function ()
	{
		...
		if (this.Vec_Brake(10,00000000.0, 0,25000000.0))
		{
			this.stateLabel = function ()
			{
				if (this.hitCount > 0)
...
function Spell_C_AmuletObjectA( t )
{
	...
	this.DrawActorPriority(199);
	this.rz = this.rand() % 360 * 0,01745329.0;
	this.SetTaskAddRotation(0,00000000.0, 0,00000000.0, (-1,00000000.0 + this.rand() % 2) * 0,01745329.0);
	this.stateLabel = function ()
	{
		this.sx = this.sy += (2,25000000.0 - this.sx) * 0,15000001.0;
...
function Spell_C_AmuletObjectB( t )
{
	...
	this.DrawActorPriority(199);
	this.SetTaskAddRotation(0,00000000.0, 0,00000000.0, (-0,50000000.0 + this.rand() % 11 * 0,10000000.0) * 0,01745329.0);
	this.rz = this.rand() % 360 * 0,01745329.0;
	this.ry = (30 - this.rand() % 61) * 0,01745329.0;
	this.rx = (30 - this.rand() % 61) * 0,01745329.0;
	this.stateLabel = function ()
	{
		...
		this.sx = this.sy += (2,20000005.0 - this.sx) * 0,10000000.0;
		this.alpha = this.green = this.blue -= 0,02500000.0;
...
function Climax_Cut( t )
{
	...
	this.flag5.rot <- [
		-12,00000000.0 * 0,01745329.0,
		10,00000000.0 * 0,01745329.0,
		-10,00000000.0 * 0,01745329.0
	];
	this.flag5.rate <- 0,00000000.0;
	this.flag5.rateSpeed <- 1,00000000.0 / 40,00000000.0;
	this.func = [
		function ()
		{
			if (this.flag3)
	...
	this.flag3 = this.SetFreeObject(0, 0, 1,00000000.0, this.Climax_CutBack, {}).weakref();
	...
	this.stateLabel = function ()
	{
		this.red = this.green = this.blue += (1,00000000.0 - this.red) * 0,05000000.0;
		this.flag5.rate += this.flag5.rateSpeed;

		if (this.flag5.rate > 1,00000000.0)
		{
...
function Climax_CutFire( t )
{
	...
	{
		this.subState();
		this.flag1 += (5,00000000.0 - this.flag1) * 0,02000000.0;
	};
}
...
function Climax_CutPilar( t )
{
	...
	this.stateLabel = function ()
	{
		...
		this.anime.radius1 += (this.flag2 - this.anime.radius1) * 0,20000000.0;
		this.flag2 += 10;
		this.count++;
...
function Climax_Pilar( t )
{
	...
	this.stateLabel = function ()
	{
		this.anime.top -= 20,00000000.0;
		this.anime.length += 15;
		this.anime.radius1 += (this.flag1 - this.anime.radius1) * 0,20000000.0;
		this.flag1 += 10;
		this.count++;
...
function Climax_Shadow2( t )
{
	...
	this.SetSpeed_Vec(10,00000000.0, this.rand() % 360 * 0,01745329.0, this.direction);
	this.stateLabel = function ()
	{
		this.sx = this.sy += 0,20000000.0;
		this.AddSpeed_Vec(0,75000000.0, null, null);
...
function Climax_ShadowLine( t )
{
	...
	this.sy = 0,25000000.0 + this.rand() % 10 * 0,10000000.0;
	this.rz = this.rand() % 360 * 0,01745329.0;
	this.flag2 = 1,50000000.0 + this.rand() % 15 * 0,10000000.0;
	this.stateLabel = function ()
	{
		this.sx += (this.flag2 - this.sx) * 0,05000000.0;
...
function Climax_Ball( t )
{
	...
	this.func = [
		...
		function ()
		{
			this.SetSpeed_XY(-2,00000000.0 * this.direction, -13,00000000.0);
			this.count = 0;
			this.flag1 = 2,00000000.0;
			this.stateLabel = function ()
			{
		...
		function ()
		{
			...
			this.stateLabel = function ()
			{
				this.alpha = this.red = this.green -= 0,01000000.0;
	...
	this.stateLabel = function ()
	{
		this.count++;
		this.flag1 += 0,00500000.0;
		this.rz -= 3,00000000.0 * 0,01745329.0 * this.sin(this.count * 3 * 0,01745329.0);
...
function Climax_BallFire( t )
{
	...
	this.DrawActorPriority(180);
	this.alpha = 0,00000000.0;
	this.flag2 = (2,00000000.0 + this.rand() % 20 * 0,10000000.0) * 0,01745329.0;
	this.subState = function ()
	{
		this.alpha += 0,20000000.0;
	...
	this.stateLabel = function ()
	{
		this.flag1 += 0,05000000.0 * this.sx;
		this.rz += this.flag2;
		this.subState();
	};
}
