...
function Master_Spell_2_Attack_B( t )
{
	...
	this.stateLabel = function ()
	{
		if (this.target.team.sp < this.target.team.sp_max2 && this.target.team.sp + 1 >= this.target.team.sp_max2)
		{
			...
			t_.type <- 1;
			this.target.team.current.SetEffect(this.target.team.index == 0 ? 40 : 1240, 664, 1,00000000.0, ::actor.effect_class.EF_SpellCharge, t_);
		}
...
function Master_Spell_2_YakuBarrage()
{
	...
	if (this.flag5.bigCycle > 0 && this.flag5.shotCount % cycle_ == 0)
	{
		...
		this.SetShot(this.point0_x, this.point0_y, 1,00000000.0, this.Master_Spell_2_ShotBig, t_);
	}
		...
		function ()
		{
			...
			this.stateLabel = function ()
			{
				...
				if (this.flag5.shotCount > this.count && this.count % this.flag5.shotCycle == 0)
				{
					...
					{
						this.SetShot(this.point0_x, this.point0_y, 1,00000000.0, this.Boss_Shot_SL1_OD, t_);
					}
					else
					{
...
function Slave_Attack_Jyoon2B( t_ )
{
	...
	{
		local t_ = {};
		t_.rot <- i * 0,17453292.0;
		this.SetShot(this.point0_x, this.point0_y, 1,00000000.0, this.Boss_Shot_SL2_OD, t_);
	}
