function Shot_Normal( t )
{
	...
	this.func = [
		...
		function ()
		{
			...
			this.flag1 = this.va.y < 0,00000000.0 ? -0,20000000.0 : 0,20000000.0;
			this.stateLabel = function ()
			{
				...
				this.subState();
				this.AddSpeed_Vec(0,25000000.0, null, 20,00000000.0, this.direction);
			};
		}
	];
	this.subState = function ()
	{
		...
		this.rz += (r_ - this.rz) * 0,33000001.0;
		return false;
	};
	this.stateLabel = function ()
	{
...
function Shot_Front( t )
{
	...
	this.stateLabel = function ()
	{
		if (this.va.x > 0,00000000.0 && this.x > ::battle.scroll_right + 200 || this.va.x < 0,00000000.0 && this.x < ::battle.scroll_left - 200 || this.Damage_ConvertOP(this.x, this.y, 8))
		{
			this.ReleaseActor();
			return true;
		}
...
function Shot_FrontExp( t )
{
	...
	this.flag1 = 1,25000000.0 + this.rand() % 75 * 0,01000000.0;
	this.rz = this.rand() % 360 * 0,01745329.0;
	this.SetTaskAddRotation(0,00000000.0, 0,00000000.0, (-2,00000000.0 + this.rand() % 41) * 0,10000000.0 * 0,01745329.0);
	this.cancelCount = 1;
	this.keyAction = [
		null,
		function ()
...
function Shot_Barrage_Effect( t )
{
	...
	this.stateLabel = function ()
	{
		this.sx = this.sy += (12,00000000.0 - this.sx) * 0,10000000.0;
...
function Shot_Barrage( t )
{
	...
	this.atk_id = 262144;
	this.rz = this.rand() % 360 * 0,01745329.0;
	this.SetSpeed_XY(0,00000000.0, -12,00000000.0);
	this.cancelCount = 1;
	...
	this.func = [
		...
		function ()
		{
			...
			this.count = 0;
			this.flag2.SetLength(0,25000000.0);
			this.subState = function ()
			{
				this.count--;
				...
				if (this.count <= 0)
				{
					...
					this.SetSpeed_XY(0,00000000.0, 0,00000000.0);
					this.flag1 = (40 - this.rand() % 81) * 0,10000000.0 * 0,01745329.0;
					this.subState = function ()
					{
	...
	this.subState = function ()
	{
		this.VY_Brake(0,25000000.0, -0,20000000.0);
		this.count++;
	};
	this.stateLabel = function ()
	{
	...
	this.func = [
		...
		function ()
		{
			...
			this.stateLabel = function ()
			{
				...
				{
					this.SetSpeed_XY(this.owner.input.x > 0 ? 10,00000000.0 : -10,00000000.0, null);
				}
				...
				{
					this.SetSpeed_XY(null, this.owner.input.y > 0 ? 10,00000000.0 : -10,00000000.0);
				}
...
function Shot_OkultDoppel_A( t )
{
	...
	this.life = 1000;
	this.red = this.green = this.blue = 0,66000003.0;
	this.stateLabel = function ()
	{
	...
	this.subState = function ()
	{
		this.VX_Brake(0,25000000.0, 6,00000000.0 * this.direction);
...
function Shot_OkultDoppel_B( t )
{
	...
	this.subState = function ()
	{
		...
		else
		{
			...
			this.AddSpeed_XY(0,00000000.0, 0,10000000.0, null, 1,00000000.0);
		}
...
function Shot_Okult( t )
{
	...
	{
		this.count++;
		this.sx += (1,00000000.0 - this.sx) * 0,05000000.0;
		...
		function ()
		{
			...
			this.subState = function ()
			{
				...
				if (this.PosHoming(this.flag2, this.flag3, this.direction) || this.count >= 45)
				{
					this.subState = function ()
					{
						this.AddSpeed_Vec(0,50000000.0, null, 15,00000000.0, this.direction);
					};
					return;
				}

				this.rz = this.atan2(this.va.y, this.va.x * this.direction);
				this.AddSpeed_Vec(0,50000000.0, null, 12,50000000.0, this.direction);
			};
		}
	];
	this.subState = function ()
	{
		this.PosHoming(this.flag2, 0,75000000.0 * 0,01745329.0, this.direction);
		this.rz = this.atan2(this.va.y, this.va.x * this.direction);
		...
		if (this.Vec_Brake(0,40000001.0, 3,00000000.0))
		{
			this.subState = function ()
			{
				this.PosHoming(this.flag2, 1,25000000.0 * 0,01745329.0, this.direction);
				this.rz = this.atan2(this.va.y, this.va.x * this.direction);
				this.count++;
...
function SPShot_C( t )
{
	if (this.x < 100 && this.direction == -1,00000000.0 || this.x > 1180 && this.direction == 1,00000000.0)
	{
...
function SPShot_C_Hole( t )
{
	...
	this.atk_id = 4194304;
	this.cancelCount = 3;
	this.SetSpeed_Vec(35,00000000.0, -70 * 0,01745329.0, this.direction);
...
function SPShot_C_Splash( t )
{
	...
	this.rz += (-2 + this.rand() % 5) * 0,01745329.0;
	this.sx -= (20 - this.rand() % 41) * 0,01000000.0;
	this.SetSpeed_Vec(60,00000000.0, -70 * 0,01745329.0, this.direction);
	this.SetCollisionRotation(0,00000000.0, 0,00000000.0, 20 * 0,01745329.0);
	this.cancelCount = 1;
...
function SPShot_D( t )
{
	...
	this.func = [
		...
		{
			this.callbackGroup = 0;
			this.flag1 = 0,30000001.0 + this.rand() % 4 * 0,10000000.0;
			this.SetSpeed_XY(0,00000000.0, 0,00000000.0);
			this.stateLabel = function ()
			{
		...
		function ( charge_ )
		{
			...
			this.hitCount = 0;
			this.grazeCount = 0;
			this.flag1.pos.SetLength(1,50000000.0);

			if (charge_)
			{
				this.SetSpeed_XY((this.flag1.pos.x + 25,00000000.0) * this.direction, this.flag1.pos.y);
			}
			else
			{
				this.SetSpeed_XY((this.flag1.pos.x + 13,50000000.0) * this.direction, this.flag1.pos.y);
			}

			this.SetTaskAddRotation(0,00000000.0, 0,00000000.0, (-5 + this.rand() % 11) * 0,01745329.0);
			this.stateLabel = function ()
			{
	...
	this.stateLabel = function ()
	{
		...
		{
			if (this.flag1.pos.y > 0,00000000.0)
			{
				this.DrawActorPriority(200);
			}
		}
		else if (this.flag1.pos.y < 0,00000000.0)
		{
			this.DrawActorPriority(180);
		}

		this.flag2 += (this.flag1.y - this.flag2) * 0,20000000.0;
...
function SPShot_E( t )
{
	...
	this.func = [
		function ()
		{
			...
			{
				this.AddSpeed_XY(0,00000000.0, 0,75000000.0);
				this.rz += (90 * 0,01745329.0 - this.rz) * 0,07500000.0;
				this.alpha = this.red = this.green = this.blue -= 0,05000000.0;
		...
		function ()
		{
			...
			this.SetSpeed_Vec(20,00000000.0, this.flag3, this.direction);
			this.SetParent(null, 0, 0);
			this.stateLabel = function ()
			{
		...
		function ()
		{
			...
			this.hitCount = 0;
			this.cancelCount = 3;
			this.flag1 = -17,50000000.0 * 0,01745329.0;
			this.SetMotion(3049, 2);
			this.SetSpeed_XY(3,00000000.0 * this.direction, -17,00000000.0);
			...
			this.stateLabel = function ()
			{
				...
				this.rz += this.flag1;
				this.AddSpeed_XY(0,00000000.0, 0,60000002.0, null, 17,00000000.0);
				this.count++;

				if (this.keyTake != 5 && this.va.y > 0,00000000.0)
				{
	...
	this.stateLabel = function ()
	{
		this.rz += (-this.rz - 25 * 0,01745329.0) * 0,25000000.0;
		this.alpha = this.red = this.green = this.blue += 0,10000000.0;
		...
		this.SetSpeed_XY((this.owner.point0_x - this.x) * 0,15000001.0, (this.owner.point0_y - this.y) * 0,15000001.0);
	};
}
...
function SpellShot_A_Hit( t )
{
	...
	this.stateLabel = function ()
	{
		local s_ = (3,00000000.0 - this.sx) * 0,25000000.0;
...
function SpellShot_B_Bullet( t )
{
	...
	this.func = [
		...
		function ()
		{
			...
			this.SetSpeed_XY(this.flag1.x * 0,25000000.0, this.flag1.y * 0,25000000.0);
			this.stateLabel = function ()
			{
				if (this.IsScreen(100))
				{
...
function SpellShot_B( t )
{
	...
	this.stateLabel = function ()
	{
		...
		{
			this.flag1.RotateByDegree(9,00000000.0);
		...
		{
			this.AddSpeed_XY(this.owner.input.x > 0 ? 1,00000000.0 : -1,00000000.0, null);
		}

		if (this.owner.input.y)
		{
			this.AddSpeed_XY(null, this.owner.input.y > 0 ? 1,00000000.0 : -1,00000000.0);
		}
	};
}
...
function SpellShot_B1( t )
{
	...
	this.stateLabel = function ()
	{
		...
		this.flag1.x = this.flag1.y += this.flag2 - this.flag1.x - 0,10000000.0;
	};
}
...
function SpellShot_B3( t )
{
	...
	this.alpha = 0,00000000.0;
	this.flag1 = this.Vector3();
	this.flag1.x = 1,75000000.0;
	this.flag1.y = 1,75000000.0;
	this.func = [
		function ()
		{
			this.stateLabel = function ()
	...
	this.stateLabel = function ()
	{
		this.alpha += this.alpha <= 0,94999999.0 ? 0,05000000.0 : 0,00000000.0;
		this.anime.top += 10,00000000.0;
	};
}
...
function SpellShot_B_Spark( t )
{
	...
	this.rz = t.rot;
	this.sx = this.sy = 0,50000000.0 + this.rand() % 15 * 0,10000000.0;
	this.keyAction = this.ReleaseActor;
	this.stateLabel = function ()
	{
...
function Climax_Shot( t )
{
	...
	this.rz = t.rot;
	this.sx = this.sy = 0,00000000.0;
	this.flag2 = 1,00000000.0 + this.rand() % 20 * 0,10000000.0;
	this.flag1 = 0,02500000.0 + this.rand() % 25 * 0,00100000.0;
	this.SetSpeed_Vec(2,00000000.0 + this.rand() % 6, this.rz, this.direction);
	this.stateLabel = function ()
	{
...
function ClimaxEffect_SceneA_Sumireko( t )
{
	...
	this.SetSpeed_XY(0,10000000.0, -0,07500000.0);
	this.func = [
		function ()
		{
			this.ReleaseActor();
...
function ClimaxEffect_SceneA_Vortex( t )
{
	...
	this.SetSpeed_XY(-0,25000000.0, 0,05000000.0);
	this.func = [
		function ()
		{
			this.ReleaseActor();
	...
	this.anime.vertex_alpha1 = 0,00000000.0;
	this.alpha = 1,00000000.0;
	this.anime.vertex_alpha0 = 1,00000000.0;
	this.anime.vertex_blue0 = 1,00000000.0;
	this.anime.vertex_red0 = 1,00000000.0;
	this.anime.vertex_green0 = 0,20000000.0;
	this.anime.vertex_alpha1 = 0,00000000.0;
...
function ClimaxEffect_SceneA_LightPilarB( t )
{
	this.SetMotion(4909, 5);
	this.DrawActorPriority(500);
	this.SetSpeed_XY(-0,25000000.0, -0,15000001.0);
	this.func = [
		function ()
		{
			this.ReleaseActor();
...
function Climax_FaceA( t )
{
	...
	this.stateLabel = function ()
	{
		...
		if (this.flag3)
		{
			...
			{
				this.flag3.alpha -= 0,05000000.0;

				if (this.flag3.alpha <= 0,00000000.0)
				{
					this.ReleaseActor.call(this.flag3);
				}
			}
...
function ClimaxCut_A( t )
{
	...
	this.SetSpeed_XY(0,15000001.0, -0,15000001.0);
	this.func = [
		function ()
		{
			this.ReleaseActor();
...
function Climax_FaceB( t )
{
	...
	this.stateLabel = function ()
	{
		this.sx = this.sy += (0,75000000.0 - this.sx) * 0,05000000.0;
		this.rz += (45 * 0,01745329.0 - this.rz) * 0,05000000.0;
		this.x += (640 + 100 * this.direction - this.x) * 0,05000000.0;
		this.y += (360 + 10 - this.y) * 0,05000000.0;
	};
	this.flag1 = null;
	this.func = [
		function ()
		{
			...
			this.stateLabel = function ()
			{
				this.x += (640 - 200 * this.direction - this.x) * 0,10000000.0;
				this.y += (360 - 100 - this.y) * 0,10000000.0;
				this.sx = this.sy += (1,25000000.0 - this.sx) * 0,10000000.0;
				this.rz += (-25 * 0,01745329.0 - this.rz) * 0,10000000.0;
				this.flag1.x = this.x;
				this.flag1.y = this.y;
	...
	this.func = [
		...
		function ()
		{
			this.stateLabel = function ()
			{
				...
				this.alpha += (1,00000000.0 - this.alpha) * 0,05000000.0;
			};
		}
	];
	this.alpha = 0,00000000.0;
...
function ClimaxEffect_BallRay( t )
{
	this.SetMotion(4908, 2);
	this.alpha = 0,00000000.0;
	this.rz = this.atan2(700 + this.y, (this.x - 640) * this.direction) - 90 * 0,01745329.0;
	this.func = [
		function ()
		{
			this.ReleaseActor();
...
function Climax_ShotLaserBottom( t )
{
	...
	this.sy = 0,75000000.0 + this.rand() % 21 * 0,01000000.0;
	this.sx = 1,00000000.0 + this.rand() % 30 * 0,01000000.0;
	this.flag1 = 0,20000000.0;
	this.stateLabel = function ()
	{
		this.sy += 0,01500000.0;
		this.sx += this.flag1;
		this.flag1 -= 0,03000000.0;

		if (this.flag1 <= 0,00500000.0)
		{
			this.flag1 = 0,00500000.0;
		}
...
function Climax_ShotLaser( t )
{
	...
	this.stateLabel = function ()
	{
		this.sx += (1,00000000.0 - this.sx) * 0,33000001.0;
		this.HitCycleUpdate(10);
