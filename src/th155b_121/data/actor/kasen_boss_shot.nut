				...
				if (this.flag1.Length() <= 150)
				{
					...
					case 4:
						...
						for( local i = 0; i < 8; i++ )
						{
							local t_ = {};
							t_.v <- 4,00000000.0;
							t_.rot <- (22,50000000.0 + i * 45) * 0,01745329.0;
							this.SetShot(this.flag3.x, this.flag3.y, this.direction, this.Eagle_Shot_Boss, t_);
						}
...
function Boss_Shot_MS2( t )
{
	...
	this.flag2 = this.Vector3();
	this.flag2.x = this.va.x * 0,05000000.0;
	this.flag2.y = this.va.y * 0,05000000.0;

	if (this.owner.com_difficulty >= 4)
	{
		this.flag5 = 0,04500000.0;
	}
	else
	{
		this.flag5 = this.owner.com_difficulty * 0,01500000.0;
	}
...
function Boss_Shot_MS2_Reimu( t )
{
	...
	this.func = [
		...
		function ()
		{
			this.count = 0;
			this.hitCount = 0;
			this.flag2 = (60 - this.rand() % 121) * 0,01745329.0;
			local t_ = {};
			t_.rot <- this.flag2;
			...
			if (this.flag1 > 3)
			{
				this.subState = function ()
				{
					...
					{
						this.PlaySE(3031);
						this.SetSpeed_Vec(20,00000000.0, this.flag2, this.direction);
						this.sx = this.sy = 3,50000000.0;
					}
			...
			else
			{
				this.subState = function ()
				{
					if (this.count == 30)
					{
						this.PlaySE(3031);
						this.SetSpeed_Vec(20,00000000.0, this.flag2, this.direction);
						this.sx = this.sy = 2,75000000.0;
					}

					if (this.va.y < 0,00000000.0 && this.y < ::battle.scroll_top || this.va.y > 0,00000000.0 && this.y > ::battle.scroll_bottom)
					{
...
function Boss_Shot_MS2B( t )
{
	...
	this.func = [
		...
		function ()
		{
			...
			this.SetSpeed_XY(this.va.x * 10,00000000.0, this.va.y * 10,00000000.0);
			this.cancelCount = 3;
			this.subState = function ()
			{
				this.AddSpeed_XY(this.flag2.x * 0,20000000.0, this.flag2.y * 0,20000000.0);
				this.sx = this.sy += (this.flag3 - this.sx) * 0,10000000.0;
				this.SetCollisionScaling(this.sx, this.sy, 1,00000000.0);
			};
			this.stateLabel = function ()
			{
	...
	if (this.owner.com_difficulty >= 4)
	{
		this.flag3 = 2,75000000.0;
		this.flag5 = 0,04500000.0;
	}
	else
	{
		this.flag3 = 2,00000000.0 + this.owner.com_difficulty * 0,25000000.0;
		this.flag5 = this.owner.com_difficulty * 0,01500000.0;
	}
...
function Boss_Shot_MS3( t )
{
	...
	{
		this.atkRate_Pat = 2,00000000.0;
	}
	else
	{
		this.atkRate_Pat = 1,00000000.0 + this.owner.com_difficulty * 0,33000001.0;
	}
	...
	this.func = [
		...
		{
			this.SetParent(null, 0, 0);
			this.SetSpeed_Vec(3,00000000.0, this.initTable.rot, this.direction);
			this.stateLabel = function ()
			{
				this.cancelCount = 99;
				local s_ = (this.initTable.scale - this.sx) * 0,10000000.0;
				this.sx = this.sy += s_;
				this.SetCollisionScaling(this.sx, this.sy, 1,00000000.0);
		...
		function ()
		{
			...
			this.SetSpeed_Vec(6,00000000.0, 1,04719746.0, this.direction);
		...
		{
			this.SetMotion(4969, 2);
			this.SetSpeed_Vec(6,00000000.0, 0,26179937.0, this.direction);
			::camera.Shake(10,00000000.0);
			this.stateLabel = function ()
			{
				if (this.IsScreen(200 * this.sx))
...
function Boss_Shot_DS2( t )
{
	...
	this.func = [
		...
		function ()
		{
			...
			r_ = this.Math_MinMax(r_, -75 * 0,01745329.0, -0,52359873.0);
			this.SetSpeed_Vec(4,00000000.0, r_, this.direction);
			this.count = 0;
			this.stateLabel = function ()
			{
				if (this.IsScreen(0))
		...
		function ()
		{
			...
			this.SetMotion(4969, 2);
			local r_ = this.atan2(this.owner.target.y - this.y, (this.owner.target.x - this.x) * this.direction);
			r_ = this.Math_MinMax(r_, 0,17453292.0, 1,04719746.0);
			this.SetSpeed_Vec(4,00000000.0, r_, this.direction);
			this.count = 0;
			this.stateLabel = function ()
			{
	...
	{
		this.cancelCount = 99;
		local s_ = (this.initTable.scale - this.sx) * 0,05000000.0;
		this.sx = this.sy += s_;
		this.SetCollisionScaling(this.sx, this.sy, 1,00000000.0);
		this.SetSpeed_Vec(s_ * 60, this.initTable.rot, this.direction);
		this.HitCycleUpdate(30);
		...
		if (this.flag1 > 0 && this.count % this.flag1 == 1)
		{
			...
			t_.v <- 5,00000000.0 + this.rand() % 31 * 0,10000000.0;
			t_.scale <- this.sx;
...
function Boss_Shot_DS3( t )
{
	...
	this.func = [
		...
		function ()
		{
			this.SetParent(null, 0, 0);
			this.SetMotion(4969, 2);
			this.SetSpeed_Vec(4,00000000.0, 0,78539813.0, this.direction);
			this.count = 0;
			this.stateLabel = function ()
			{
				if (this.IsScreen(0))
				...
				if (this.flag1 > 0 && this.count % this.flag1 == 1)
				{
					...
					t_.scale <- this.sx;
					this.SetShot(this.x, this.y, this.direction, this.Boss_Shot_DS2_Ball, t_);
					this.flag2 += (15 + this.rand() % 6) * 0,01745329.0;
				}
			};
		}
	];
	...
	{
		this.cancelCount = 99;
		local s_ = (3,50000000.0 - this.sx) * 0,05000000.0;
		this.sx = this.sy += s_;
		this.SetCollisionScaling(this.sx, this.sy, 1,00000000.0);
		this.SetSpeed_Vec(s_ * 60, this.initTable.rot, this.direction);
		this.HitCycleUpdate(30);
		...
		if (this.flag1 > 0 && this.count % this.flag1 == 1)
		{
			...
			this.flag2 += (15 + this.rand() % 3) * 0,01745329.0;
		}
	};
}
