function SetCamera()
{
	::camera.func_update_3d = function ( camera )
	{
		...
		local t = (-(camera.camera2d.left + camera.camera2d.right) / 2 + 640) * 0,00050000.0;
		this.eye_x = (-(camera.camera2d.left + camera.camera2d.right) / 2 + 640) * 1;
		this.at_y = ((camera.camera2d.top + camera.camera2d.bottom) / 1,50000000.0 - (560 + 300)) * 1;
...
this.bloom_filter.InitBloom(this.src_target, 128, 1, 30,00000000.0, 2,00000000.0, 0,20000000.0, 0,69999999.0);
