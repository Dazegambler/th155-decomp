function SetCamera()
{
	::camera.func_update_3d = function ( camera )
	{
		...
		this.at_y = (camera.camera2d.top + camera.camera2d.bottom) / 2 * 1;
		this.eye_y = this.at_y;
		local w = camera.camera2d.right - camera.camera2d.left;
		this.eye_z = -1900,00000000.0;
		this.fov = this.atan(w / -this.eye_z) * 1,00000000.0;
		this.z_far = 300000;
...
this.animation_set.Load(0, "data/background/bg41/stage01.msh", null);
...
this.animation_set.Load(10, "data/background/bg41/stagestone.msh", null);
...
this.bloom_filter.InitBloom(this.src_target, 128, 1, 50,00000000.0, 1,00000000.0, 0,10000000.0, 0,80000001.0);
