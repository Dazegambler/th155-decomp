...
function Rush_Smash()
{
	if (this.Cancel_Check(40))
	{
		if (this.command.rsv_y < 0)
		{
			...
			{
				this.combo_func = this.Rush_Skill_Resurrection;
				return true;
			}
		}
		else if (this.command.rsv_y > 0)
		{
			...
			{
				this.combo_func = this.Rush_Skill_Kick;
				return true;
			}
		}
		else if (this.Atk_RushC_Front_Init(null))
		{
			this.combo_func = this.Rush_Skill_Clow;
			return true;
		}
	}
...
function Rush_Far()
{
	if (this.command.rsv_y == 0 && this.command.rsv_x * this.direction <= 0)
	{
		...
		{
			this.combo_func = this.Rush_Shot_Under;
			return true;
		}
	}
...
function Rush_Skill_Resurrection()
{
	if (this.team.mp >= 200)
	{
		if (this.Cancel_Check(40))
