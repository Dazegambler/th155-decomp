		...
		if (this.centerStop * this.centerStop <= 1)
		{
			...
			if (d_ <= 125)
			{
				...
				]))
				{
					...
					if (this.com_rand[3] <= 90 && (this.target.IsDamage() || this.target.IsGuard()))
					{
						this.CPU_AtkLow(-60);
						return;
					}
			...
			if (d_ <= 225)
			{
				...
				]))
				{
					...
					if (this.com_rand[3] <= 60 && this.com_enemyPos == 13)
					{
						this.CPU_SkillD(-60);
						return;
					}
		...
		else
		{
			if (d_ <= 175)
			{
				...
				]))
				{
					...
					if (this.targetHeight - this.y > 0 && this.targetHeight - this.y < 125)
					{
...
function CPU_AtkLow( wait_ )
{
	...
	if (this.centerStop * this.centerStop <= 1)
	{
		this.com_subState[3] = function ()
		{
			...
			if (this.abs(this.targetDist) <= 120)
			{
	...
	if (this.centerStop * this.centerStop <= 1)
	{
		if (this.atkRange > this.abs(this.targetDist))
		{
			this.com_subState[3] = function ()
			{
				...
				if (this.motion >= 1100 && this.motion <= 1109 && this.keyTake >= 3 || this.motion == 1000)
				{
					if (this.com_rand[3] <= 3)
					{
						this.CPU_ChainShot(-50);
...
function CPU_AtkUpper( wait_ )
{
	...
	this.com_subState[3] = function ()
	{
		...
		if (this.motion >= 1220 && this.motion <= 1229)
		{
			if (this.hitResult & 1)
			{
				if (this.com_rand[3] <= 5 && this.team.spell_active && this.spellcard.id == 0)
				{
					this.CPU_SpellA(-60);
					return;
				}
...
function CPU_AtkUnder( wait_ )
{
	...
	this.com_subState[3] = function ()
	{
		...
		if (this.motion >= 1210 && this.motion <= 1219)
		{
			if (this.hitResult & 1)
			{
				if (this.com_rand[3] <= 5 && this.team.spell_active && this.spellcard.id == 0)
			...
			if (this.keyTake == 1 && this.keyFrame >= 2)
			{
				if (this.com_rand[3] <= 8)
				{
...
function CPU_AtkFront( wait_ )
{
	...
	this.com_subState[3] = function ()
	{
		...
		{
			if (this.hitResult & 1 && this.keyFrame >= 3)
			{
				if (this.com_rand[3] <= 5 && this.team.spell_active && this.spellcard.id == 2)
				{
					this.CPU_SkillC(-60);
					return;
				}
...
function CPU_AtkDashLow( wait_ )
{
	...
	if (this.centerStop * this.centerStop <= 1)
	{
		this.com_subState[3] = function ()
		{
			...
			else if (this.motion <= 1300 && this.motion <= 1309 && this.keyTake >= 1)
			{
				if (this.hitResult & 1)
				{
					if (this.com_rand[3] <= 9)
				...
				if (this.keyFrame >= 1 && this.com_rand[3] <= 4)
				{
					this.CPU_ChainSkill(-30);
				}
		...
		this.com_subState[3] = function ()
		{
			...
			if (this.motion == 42)
			{
				if (this.CPU_CheckEnemyPos([
					...
					5,
					8
				]) && (this.target.y < this.y && this.va.y < 0 || this.target.y > this.y && this.va.y > 0))
				{
					this.command.rsv_k0 = 3;
					this.command.rsv_x = 0;
				}
			}
			else if (this.motion <= 1110 && this.motion <= 1119 && this.keyTake >= 1)
			{
				if (this.com_rand[3] <= 3)
				{
					this.CPU_ChainSkill(-30);
...
function CPU_AtkDashHigh( wait_ )
{
	...
	this.com_subState[3] = function ()
	{
		...
		if (this.motion <= 1310 && this.motion <= 1319)
		{
			return true;
		}
	};
	...
	this.com_subState[3] = function ()
	{
		...
		if (this.com_count[3] >= -10 || this.motion == 2025)
		{
			this.com_subState[3] = function ()
			{
				...
				if (this.motion == 2020 && this.keyTake == 3)
				{
					if (this.com_rand[3] <= 8)
					{
						this.CPU_ChainMove(-60);
...
function CPU_SkillA( wait_ )
{
	...
	this.com_subState[3] = function ()
	{
		...
		if (this.motion >= 3010 && this.motion <= 3019 && this.keyTake >= 2)
		{
			if (this.com_rand[3] <= 9)
			{
				this.CPU_ChainMove(-60);
...
function CPU_SkillC( wait_ )
{
	...
	this.com_subState[3] = function ()
	{
		...
		{
			if (this.keyTake >= 2 && this.com_rand[3] <= 9)
			{
				this.CPU_ChainMove(-60);
				return true;
			}
...
function CPU_SkillD( wait_ )
{
	...
	this.com_subState[3] = function ()
	{
		...
		this.command.rsv_k2 = 1;
		this.command.rsv_x = (this.target.x - this.x) * this.direction > 400 ? this.direction : 0;
		this.command.rsv_y = 1;

		if (this.motion >= 3060 && this.motion <= 3069)
		{
			if (this.keyTake >= 2 && this.com_rand[3] <= 9)
			{
				this.CPU_ChainMove(-60);
...
function CPU_ChainSpell( wait_ )
{
	...
	this.com_subState[1] = function ()
	{
		...
		{
		case 0:
			if (this.y < this.target.y - 75 && this.target.IsDamage() && this.target.recover >= 25)
			{
				this.CPU_SpellA(-30);
				return true;
			}
		...
		case 1:
			if (this.target.IsDamage() && this.target.recover >= 60 && this.abs(this.targetDist) <= 350)
			{
				this.CPU_SpellB(-30);
				return true;
			}
...
function CPU_CheckSpell()
{
	...
	{
	case 0:
		if (this.y < this.target.y - 75 && this.rand() % 100 <= 2 || this.target.IsAttack() == 2 && this.rand() % 100 <= 10)
		{
			this.CPU_SpellA(-60);
			return true;
		}
	...
	case 1:
		if (this.abs(this.targetDist) <= 400)
		{
			if (this.rand() % 100 <= 2)
			{
				this.CPU_SpellB(-60);
			...
			if (this.rand() % 100 <= 5 && this.CPU_CheckEnemyPos([
				18,
				28,
				26,
				13,
