...
function Rush_Smash()
{
	if (this.Cancel_Check(40))
	{
		if (this.command.rsv_y < 0)
		{
			...
			{
				this.combo_func = this.Rush_Skill_Mizubaku;
				return true;
			}
		}
		else if (this.command.rsv_y > 0)
		{
			...
			{
				this.combo_func = this.Rush_Skill_Kiku;
				return true;
			}
		}
		else if (this.Atk_RushC_Front_Init(null))
		{
			this.combo_func = this.Rush_Skill_Drill;
			return true;
		}
	}
...
function Rush_Far()
{
	if (this.command.rsv_y == 0 && this.command.rsv_x * this.direction <= 0)
	{
		...
		{
			this.combo_func = this.Rush_Shot_Normal;
			return true;
		}
	}
...
function Rush_Air()
{
	if (this.centerStop * this.centerStop >= 4 && this.command.rsv_y == 0 && this.command.rsv_x * this.direction <= 0 && this.Cancel_Check(40) && this.Atk_RushA_Air_Init(null))
	{
		...
		{
			this.combo_func = this.Rush_Skill_Rubber;
			return true;
		}
		else
		{
			this.combo_func = this.Rush_Skill_Mizubaku_NoRush;
			return true;
		}
	}
...
function Rush_Upper()
{
	if (this.team.mp >= 200 && this.centerStop * this.centerStop >= 4 && this.Cancel_Check(40))
	{
		...
		{
			this.combo_func = this.Rush_Skill_Blaster;
			return true;
		}
	}
...
function Rush_Shot_Under_Air()
{
	if (this.team.mp >= 200 && this.centerStop * this.centerStop >= 4 && this.Cancel_Check(40))
	{
		...
		{
			this.combo_func = this.Rush_Skill_Torpedo;
			return true;
		}
	}
}
...
function Rush_Skill_Mizubaku_NoRush()
{
	if (this.team.mp >= 200)
	{
		if (this.Cancel_Check(50))
...
function Rush_Skill_Torpedo()
{
	if (this.team.mp >= 200)
	{
		if (this.centerStop * this.centerStop <= 1)
		{
			if (this.hitResult & 8)
			{
				if (this.Cancel_Check(40) && this.Shot_Normal_Init(null))
				{
					return true;
				}
			}
		}
		else if (this.hitResult & 8)
		{
			if (this.Cancel_Check(40) && this.Shot_Normal_Init(null))
			{
				return true;
