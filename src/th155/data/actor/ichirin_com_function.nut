...
function CPU_SkillB( wait_ )
{
	...
	this.com_subState[3] = function ()
	{
		if (this.com_count[3] > 0 || this.centerStop * this.centerStop >= 2 && this.y > this.centerY)
		{
			return true;
		}
		...
		if (this.motion == 3010 || this.motion == 3070)
		{
			if (this.com_rand[3] <= 9)
			{
				this.CPU_ChainMove(-60);
...
{
	if (this.team.op < 1000 || this.team.op_stop > 0 || this.team.master != this)
	{
		return true;
	}
...
function CPU_ChainSpell( wait_ )
{
	...
	this.com_subState[1] = function ()
	{
		...
		case 2:
			if (this.target.IsDamage() && this.target.recover >= 30 && this.abs(this.targetDist) <= 600)
			{
				this.CPU_SpellC(-30);
				return true;
			}
...
function CPU_CheckSpell()
{
	...
	{
	case 0:
		if (this.rand() % 100 <= 2 || this.target.IsAttack() == 2 && this.rand() % 100 <= 10 && this.abs(this.targetDist) >= 300 && this.abs(this.targetDist) <= 600)
		{
			this.CPU_SpellA(-60);
			return true;
		}
	...
	this.com_subState[3] = function ()
	{
		...
		if (this.motion == 4040)
		{
			this.com_subState[3] = function ()
			{
				...
				if (this.abs(this.targetHeight) <= 50)
				{
					this.input.y = 0;

					if (this.abs(this.targetDist) <= 150)
					{
						this.input.x = 0;
					}
					else if (this.targetDist > 0)
					{
						this.input.x = 1;
					}
					else
