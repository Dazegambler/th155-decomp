...
function Team_Change_ShotB( va_ )
{
	...
	this.keyAction = [
		...
		function ()
		{
			...
			this.flag1 = this.Math_MinMax(this.flag1, -50 * 0,01745329.0, 50 * 0,01745329.0);

			for( local i = 0; i < 7; i++ )
			{
				local t_ = {};
				t_.rot <- this.flag1 + 0,26179937.0 + i * 0,17453292.0;
				t_.wait <- 8 + i * 4;
				this.SetShot(this.point0_x + 75 * this.cos(t_.rot) * this.direction, this.point0_y + 75 * this.sin(t_.rot), this.direction, this.Shot_Change, t_);
				local t_ = {};
				t_.rot <- this.flag1 - 0,26179937.0 - i * 0,17453292.0;
				t_.wait <- 8 + i * 4;
				this.SetShot(this.point0_x + 75 * this.cos(t_.rot) * this.direction, this.point0_y + 75 * this.sin(t_.rot), this.direction, this.Shot_Change, t_);
