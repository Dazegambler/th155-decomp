...
{
	local t_ = {};
	t_.speed <- 4,50000000.0;
	t_.addSpeed <- 0,20000000.0;
	t_.maxSpeed <- 12,50000000.0;
	t_.wait <- 120;
...
function Atk_HighUnder_Init( t )
{
	...
	this.keyAction = [
		...
		{
			this.PlaySE(3010);
			this.SetSpeed_XY(-10,00000000.0 * this.direction, -7,00000000.0);
			this.centerStop = -2;
			this.stateLabel = function ()
			{
				this.VX_Brake(0,25000000.0);
		...
		function ()
		{
			...
			this.stateLabel = function ()
			{
				if (this.flag1)
				{
					...
					if (this.count >= 20 && (this.input.b0 == 0 || this.input.b1 == 0) || this.flag1.cancelCount <= 0 || this.flag1.hitCount > 0 || this.count > 60 || this.flag1.va.x > 0,00000000.0 && this.flag1.x > ::battle.scroll_right || this.flag1.va.x < 0,00000000.0 && this.flag1.x < ::battle.scroll_left)
					{
...
function SP_B2_Init( t )
{
	...
	this.stateLabel = function ()
	{
		...
		else if (this.VX_Brake(0,15000001.0, -2,00000000.0 * this.direction))
		{
			this.flag1 = true;
		}
	...
	this.keyAction = [
		function ()
		{
			...
			if (this.ball)
			{
				...
				this.SetShotStencil(this.ball.x + this.ball.va.x * 15, y_, this.direction, this.SPShot_D, {});
			}
			else
			{
