...
function CPU_AtkLow( wait_ )
{
	...
	if (this.centerStop * this.centerStop <= 1)
	{
		this.com_subState[3] = function ()
		{
			...
			if (this.abs(this.targetDist) <= 65)
			{
	...
	this.com_subState[3] = function ()
	{
		...
		if (this.com_count[3] >= -10 || this.motion == 2025)
		{
			this.com_subState[3] = function ()
			{
				...
				if (this.motion == 2020 && this.keyTake == 2)
				{
					if (this.com_rand[3] <= 8)
					{
						this.CPU_ChainMove(-60);
...
function CPU_ChainSpell( wait_ )
{
	...
	this.com_subState[1] = function ()
	{
		...
		{
		case 0:
			if (this.target.IsDamage() && this.target.recover >= 35 && this.CPU_CheckEnemyPos([
				5,
				16
			]))
			{
		...
		case 1:
			if (this.abs(this.targetDist) <= 90 && this.abs(this.targetHeight) <= 50)
			{
		...
		case 2:
			if (this.target.IsDamage() && this.target.recover >= 40)
			{
				if (this.CPU_CheckEnemyPos([
					13,
					23
...
function CPU_CheckSpell()
{
	...
	{
	case 0:
		if ((this.rand() % 100 <= 2 || this.target.IsAttack() == 2 && this.rand() % 100 <= 10) && this.CPU_CheckEnemyPos([
			5,
			16
		]))
		{
