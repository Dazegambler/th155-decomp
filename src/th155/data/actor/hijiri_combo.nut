...
function Rush_Smash()
{
	if (this.Cancel_Check(40))
	{
		if (this.command.rsv_y < 0)
		{
			...
			{
				this.combo_func = this.Rush_Skill_N_Air;
				return true;
			}
		}
...
function Rush_Far()
{
	if (this.command.rsv_y == 0 && this.command.rsv_x * this.direction <= 0)
	{
		...
		{
			this.combo_func = this.Rush_Shot_Normal_Under;
			return true;
		}
	}
...
function Rush_Air()
{
	...
	{
		this.combo_func = this.Rush_Skill_Back_Air;
		return true;
	}
...
function Rush_Shot_Normal_Under()
{
	if (this.team.mp >= 200 && this.Cancel_Check(40))
	{
		if (this.hitResult & 8)
