...
function CPU_SkillC( wait_ )
{
	...
	this.com_subState[3] = function ()
	{
		...
		if (this.motion == 3040 || this.motion == 3041)
		{
			if (this.com_rand[3] <= 9)
			{
				this.CPU_ChainMove(-60);
...
function CPU_ChainSpell( wait_ )
{
	...
	this.com_subState[1] = function ()
	{
		...
		{
		case 0:
			if (this.target.IsDamage() && this.target.recover >= 35 && this.abs(this.targetHeight) <= 50)
			{
				this.CPU_SpellA(-60);
				return true;
			}

			if (this.com_rand[1] <= 2 && this.abs(this.targetHeight) <= 50)
			{
				this.CPU_SpellA(-60);
				return true;
			}
